...
exports.narrativesForProvider = narrativesForProvider;
exports.computeSignalAssessment = computeSignalAssessment;
exports.buildNarrativeClusters = buildNarrativeClusters;
...
function narrativesForProvider(provider) {
...
function computeSignalAssessment(provider, allProviders, assessedAt = new Date().toISOString()) {
    const categoryProviders = allProviders.filter((item) => item.category === provider.category);
    const categorySizes = allProviders.reduce((acc, item) => {
...
function buildNarrativeClusters(providers, signals) {
    return Object.entries(narrativeKeywords).map(([title, keywords]) => {
        ...
        const knownScores = memberSignals.map((signal) => signal.score).filter((score) => score !== null);
        const heat = knownScores.length ? clamp(knownScores.reduce((sum, score) => sum + score, 0) / knownScores.length) : null;
        const momentumValues = memberSignals.map((signal) => signal.components.ecosystemMomentum).filter((score) => score !== null);
