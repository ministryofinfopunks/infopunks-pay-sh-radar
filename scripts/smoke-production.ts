...
export function buildSmokePlan(): SmokePlan {
  ...
  return {
    publicPaths: [
      ...
      '/developers/wallet-safety/integrations/agent_wallet_demo',
      '/developers/wallet-safety/integrations/x402_service_router',
      '/spend-terminal',
      '/routes',
      '/providers',
      '/services',
      ...
      '/rh-chain-signal-desk/tokens/0x1111111111111111111111111111111111111111',
    ...
    apiGetPaths: [
      ...
      '/v1/hermes/wallet-safety/integrations',
      '/v1/hermes/wallet-safety/integrations/agent_wallet_demo/readiness',
      '/v1/hermes/wallet-safety/integrations/agent_wallet_demo',
      ...
      `/v1/hermes/reputation-ledger/provider/${encodeURIComponent(providerId)}`,
      ...
      '/v1/rh-chain/memes',
      '/v1/rh-chain/signals',
      '/v1/rh-chain/receipts',
      '/v1/rh-chain/4663-index',
...
function assertHermesDecisionReceipt(body: unknown): void {
  ...
  if (!isRecord(data.receipt)) throw new Error('missing data.receipt');
  if (!isRecord(data.conversion)) throw new Error('missing data.conversion');
  if (data.receipt.receipt_kind !== 'pre_spend_decision_receipt') throw new Error('unexpected decision receipt kind');
}

function assertHermesPolicyDecisionReceipt(body: unknown): void {
  if (!hasDataEnvelope(body) || !isRecord(body.data)) throw new Error('missing Hermes policy decision receipt payload');
  const data = body.data;
  if (!isRecord(data.receipt)) throw new Error('missing data.receipt');
  if (!isRecord(data.conversion)) throw new Error('missing data.conversion');
  if (!isRecord(data.receipt.audit_trail)) throw new Error('missing data.receipt.audit_trail');
  if (!isRecord(data.receipt.risk_summary)) throw new Error('missing data.receipt.risk_summary');
...
function assertHermesDecisionOutcome(body: unknown): void {
  ...
  if (!isRecord(data.receipt)) throw new Error('missing data.receipt');
  if (!isRecord(data.outcome)) throw new Error('missing data.outcome');
  if (!isRecord(data.reputation_feedback)) throw new Error('missing data.reputation_feedback');
}
...
export async function runSmoke(baseUrl = resolveBaseUrl(), config = resolveSmokeConfig()): Promise<boolean> {
  ...
  try {
    const { body, elapsedMs } = await checkJsonGet(baseUrl, plan.hermesSpendPolicyReceiptPreviewPath, config.apiTimeoutMs);
    assertHermesPolicyDecisionReceipt(body);
    pass(`GET ${plan.hermesSpendPolicyReceiptPreviewPath}`, elapsedMs);
  } catch (error) {
    failed = true;
    fail(`GET ${plan.hermesSpendPolicyReceiptPreviewPath}`, toFailureDetail('GET', plan.hermesSpendPolicyReceiptPreviewPath, error));
  }

  try {
    const { response, elapsedMs } = await fetchWithRetry({
      input: `${baseUrl}${plan.hermesSpendPolicyReceiptPath}`,
      method: 'POST',
      path: plan.hermesSpendPolicyReceiptPath,
      timeoutMs: config.apiTimeoutMs,
      init: {
        method: 'POST',
        headers: { accept: 'application/json' }
    ...
    const body = await parseJsonOrThrow('POST', plan.hermesSpendPolicyReceiptPath, response, elapsedMs);
    assertHermesPolicyDecisionReceipt(body);
    pass(`POST ${plan.hermesSpendPolicyReceiptPath}`, elapsedMs);
  } catch (error) {
    failed = true;
  ...
  try {
    const { response, elapsedMs } = await fetchWithRetry({
      input: `${baseUrl}${plan.hermesDecisionReceiptPath}`,
      method: 'POST',
      path: plan.hermesDecisionReceiptPath,
      timeoutMs: config.apiTimeoutMs,
      init: {
        method: 'POST',
        headers: { accept: 'application/json' }
    ...
    const body = await parseJsonOrThrow('POST', plan.hermesDecisionReceiptPath, response, elapsedMs);
    assertHermesDecisionReceipt(body);
    pass(`POST ${plan.hermesDecisionReceiptPath}`, elapsedMs);
  } catch (error) {
    failed = true;
    fail(`POST ${plan.hermesDecisionReceiptPath}`, toFailureDetail('POST', plan.hermesDecisionReceiptPath, error));
  }

  try {
    const { response, elapsedMs } = await fetchWithRetry({
      input: `${baseUrl}${plan.hermesDecisionOutcomePath}`,
      method: 'POST',
      path: plan.hermesDecisionOutcomePath,
      timeoutMs: config.apiTimeoutMs,
      init: {
        method: 'POST',
        headers: {
    ...
    const body = await parseJsonOrThrow('POST', plan.hermesDecisionOutcomePath, response, elapsedMs);
    assertHermesDecisionOutcome(body);
    pass(`POST ${plan.hermesDecisionOutcomePath}`, elapsedMs);
  } catch (error) {
    failed = true;
    fail(`POST ${plan.hermesDecisionOutcomePath}`, toFailureDetail('POST', plan.hermesDecisionOutcomePath, error));
  }
