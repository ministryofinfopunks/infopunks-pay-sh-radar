...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/rh-chain/market/provider-status', rhChain('Get DEX Screener provider status', 'Returns DEX Screener enablement, last capture, boost observation count, known-token refresh state, and fallback mode. Provider observations are context only, never endorsement or approved signals.', 'RhChainMarketProviderStatusPayload', { ...rhExample, provider: 'dexscreener', enabled: false, fallback_mode: true }));
  add('get', '/v1/rh-chain/market/tokens', rhChain('Refresh known RH Chain market context', 'Refreshes known exact-contract tokens in batches of at most 30. A provider failure fails soft and does not affect reviewed receipts or classifications.', 'RhChainMarketTokenListPayload', { ...rhExample, tokens: [], status: { provider: 'dexscreener', fallback_mode: true } }, [{ name: 'contracts', in: 'query', required: false, schema: stringSchema(), description: 'Optional comma-separated exact contracts. Tickers are not accepted as identity.' }]));
  add('get', '/v1/rh-chain/market/tokens/{contract}', rhChain('Get exact-contract market context', 'Returns raw provider observation separately from Infopunks analysis, reviewed classification, provenance, and caveats. Paid visibility is context only.', 'RhChainMarketTokenPayload', { ...rhExample, token: { contract: '0xexample' }, attention: { attention_state: 'source_required' }, classification: { source: 'review_required' } }, [pathParam('contract', 'Exact token contract address. Tickers never establish identity.')]));
  ...
  add('post', '/v1/rh-chain/market/snapshots/capture', rhChain('Capture RH Chain market snapshots', 'Admin-gated, low-frequency watchlist capture using the documented DEX Screener batch API. Disabled unless RH Chain automation or snapshot capture is enabled; provider failures write nothing.', 'RhChainMarketSnapshotCapturePayload', { ...rhExample, captured: [], status: 'disabled' }, undefined, undefined, { '401': rhChainErrorResponse('admin_token_required'), '503': rhChainErrorResponse('rh_chain_snapshot_capture_disabled') }));
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  return {
    ...
    RhChainMarketSnapshotCapturePayload: objectSchema({ captured: arrayOf(freeformObject()), status: stringSchema(), caveats: arrayOf(stringSchema()) }, ['captured', 'status', 'caveats']),
