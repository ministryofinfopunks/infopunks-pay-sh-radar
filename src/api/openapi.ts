...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('post', '/v1/4663/reflexive/preflight/ipx-pltr/simulate', rh4663('Simulate a hypothetical IPX/PLTR market', 'Consumes one explicit immutable READY_FOR_SIMULATION PLTR observation and either one hypothetical first-party IPX configuration or two-to-four comparable configurations for a comparison matrix. The pure v0.5.0 model performs concentrated-liquidity, buy/sell, supply, price, basis, session, capital/flow, route, and range stress without fetching live fields or exposing any transaction capability. Outputs are PREFLIGHT_SIMULATION_RECORD objects, never CALL or RESOLUTION receipts and never launch authorization.', 'IpxPltrPreflightSimulation', { record_type: 'PREFLIGHT_SIMULATION_RECORD', simulation_id: 'ipxsim-sha256', state_snapshot_id: 'pltr-preflight-52406504-20260902074509000', immutable: true, result: { synthetic_ipx_asset: { contract_address: null }, draft_simulation_verdict: { label: 'DRAFT_SIMULATION_VERDICT', verdict: 'DEGRADE', launch_authorization: false } } }, jsonRequest({ oneOf: [objectSchema({ state_snapshot_id: stringSchema(), hypothetical_configuration: freeformObject() }, ['state_snapshot_id', 'hypothetical_configuration']), objectSchema({ state_snapshot_id: stringSchema(), hypothetical_configurations: arrayOf(freeformObject()) }, ['state_snapshot_id', 'hypothetical_configurations'])] }, { state_snapshot_id: 'pltr-preflight-52406504-20260902074509000', hypothetical_configuration: { simulation_name: 'Illustrative anchor stress', architecture: 'PLTR_ANCHOR', asset_owner: 'INFOPUNKS', first_party_asset: true } })));
