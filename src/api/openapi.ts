...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/rh-chain/market-structure/cross-layer', rhChain('Get RH Chain cross-layer assets', 'Returns only exact-contract reviewed cross-layer classifications. Provider observations cannot create an entry.', 'RhChainCrossLayerPayload', { ...rhExample, title: 'Cross-Layer Assets', entries: [] }));
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  const rhChainClassificationLayer = enumSchema(['meme', 'rwa', 'agent', 'infrastructure', 'defi', 'unknown']);
  ...
  return {
    ...
    RhChainCrossLayerPayload: objectSchema({ title: { const: 'Cross-Layer Assets' }, entries: arrayOf(freeformObject()), categories: arrayOf(stringSchema()), observed_at: dateTimeSchema() }, ['title', 'entries', 'categories', 'observed_at']),
