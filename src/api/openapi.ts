...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  return {
    ...
    Rh4663FrontdoorState: objectSchema({ object_type: { const: 'RH_4663_FRONTDOOR_STATE' }, generated_at: dateTimeSchema(), freshness: objectSchema({ state: enumSchema(['VERIFIED', 'MIXED', 'WATCH', 'UNRESOLVED', 'BLOCK', 'DEGRADE', 'INSUFFICIENT_DATA']), source_observed_at: nullableString() }, ['state', 'source_observed_at']), frontdoor_version: objectSchema({ object_type: { const: 'FRONTDOOR_VERSION' }, version: { type: 'integer', minimum: 1 }, changed: arrayOf(stringSchema()), generated_at: dateTimeSchema() }, ['object_type', 'version', 'changed', 'generated_at']), frontdoor_version_durability: enumSchema(['PERSISTENT', 'EPHEMERAL']), now_cards: arrayOf(freeformObject()), watch_cards: arrayOf(freeformObject()), open_loops: arrayOf(freeformObject()), current_call: freeformObject(), proof_summary: freeformObject(), system_status: freeformObject(), source_refs: arrayOf(freeformObject()) }, ['object_type', 'generated_at', 'freshness', 'frontdoor_version', 'frontdoor_version_durability', 'now_cards', 'watch_cards', 'open_loops', 'current_call', 'proof_summary', 'system_status', 'source_refs']),
