...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/reflexive/pairs', rh4663('List verified Stock-quoted mission pairs', 'Lists only pairs whose quote contract matches the canonical Robinhood chain-4663 asset registry; ticker matches alone are rejected.', 'ReflexivePairList', { pairs: [], methodology_version: 'rmm-v0.3.1' }));
  ...
  add('get', '/v1/4663/reflexive/audits/long-ai-nvda', rh4663('Audit LONG AI/NVDA against Doppler and V4 core state', 'Read-only, fail-closed audit of the Airlock asset record, LongLauncher creation receipt, canonical factory, actual initializer, and every launch position currently read through StateView. Principal is scoped to LONG_CANONICAL_LAUNCH_POSITION_INVENTORY; it is never a PoolManager balance or total-pool claim.', 'ReflexiveLongAudit', { canonical_numeraire_verified: true, long_attribution_method: 'LONG_LAUNCHER_AND_AIRLOCK_INTEGRATOR', doppler_generation: 'DOPPLER_HOOK_INITIALIZER_V1', inventory_scope: 'LONG_CANONICAL_LAUNCH_POSITION_INVENTORY', inventory_status: 'AVAILABLE', external_liquidity_coverage: 'PARTIAL' }));
