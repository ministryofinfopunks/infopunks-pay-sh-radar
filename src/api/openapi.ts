...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/reflexive/stocks/PLTR/preflight', rh4663('Get PLTR preflight state', 'Returns one evidence-backed, read-only PLTR_PREFLIGHT_STATE for a future hypothetical simulator. It contains no IPX launch, trade, sizing, valuation, or ALLOW/DEGRADE/BLOCK verdict.', 'PltrPreflightState', { state_type: 'PLTR_PREFLIGHT_STATE', readiness: { status: 'PARTIAL', missing_prerequisites: ['fresh direct PLTR market state unavailable'] }, first_party_footprint: { schema: 'PROPOSED_FIRST_PARTY_PLTR_FOOTPRINT_V1', values_populated: false } }));
