...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/signals', rh4663('List 4663 Signal Cards', 'Returns public attributed Signal Cards. Signal Cards are editorial intelligence and never inherit Protocol Receipt guarantees.', 'Rh4663SignalList', { signals: [], guarantee_notice: 'Signal Cards are editorial intelligence, not Evidence Receipts or Protocol Receipts.' }));
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  return {
    ...
    Rh4663TodayEdition: objectSchema({ edition_id: stringSchema(), date: { type: 'string', format: 'date' }, generated_at: dateTimeSchema(), top_events: arrayOf(freeformObject()), category_flows: arrayOf(freeformObject()), key_signal: stringSchema(), rh_pulse_consensus: { anyOf: [freeformObject(), { type: 'null' }] }, rh_pulse: { anyOf: [freeformObject(), { type: 'null' }] }, evidence_references: arrayOf(freeformObject()), confidence: { type: 'integer', minimum: 0, maximum: 100 }, source_timestamps: arrayOf(dateTimeSchema()), provider_state: enumSchema(['available', 'stale', 'degraded', 'unavailable']), storage_status: enumSchema(['durable', 'memory', 'unavailable']), archive_path: stringSchema(), data_notice: stringSchema() }, ['edition_id', 'date', 'generated_at', 'top_events', 'category_flows', 'key_signal', 'evidence_references', 'confidence', 'source_timestamps', 'provider_state', 'storage_status', 'archive_path', 'data_notice']),
    ...
    Rh4663SignalList: objectSchema({ signals: arrayOf({ $ref: '#/components/schemas/Rh4663Signal' }), guarantee_notice: stringSchema() }, ['signals', 'guarantee_notice']),
