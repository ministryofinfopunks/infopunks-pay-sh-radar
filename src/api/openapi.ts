...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/reflexive/stocks/PLTR/preflight', rh4663('Get immutable PLTR preflight state', 'Returns a persisted v0.4.4 PLTR_PREFLIGHT_STATE observation. `observation_id` may select an exact historical state; no field is refreshed during retrieval. Provider discovery cannot satisfy direct-market, depth, or basis readiness.', 'PltrPreflightState', { observation_id: 'pltr-preflight-52371729-20260902064656000', immutable: true, state_type: 'PLTR_PREFLIGHT_STATE', readiness: { status: 'PARTIAL', missing_prerequisites: ['independently verified direct PLTR market state unavailable'] }, first_party_footprint: { schema: 'PROPOSED_FIRST_PARTY_PLTR_FOOTPRINT_V1', values_populated: false } }, undefined, [{ name: 'observation_id', in: 'query', required: false, schema: stringSchema(), description: 'Immutable preflight-state observation identifier.' }]));
