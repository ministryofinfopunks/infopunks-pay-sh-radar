...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  }), { count: 4, mappings: [{ provider_name: 'StableCrypto', benchmark_intent: 'token search', mapping_status: 'verified', execution_evidence_status: 'unproven' }, { provider_name: 'CoinGecko Onchain DEX API', benchmark_intent: 'token search', mapping_status: 'verified', execution_evidence_status: 'proven' }] }));
  ...
  add('get', '/v1/radar/benchmarks/finance-data-token-search', radarGet('Radar Readiness', 'Get token-search benchmark scaffold', 'Returns the finance/data token-search planning scaffold. benchmark_recorded=false means no normalized benchmark evidence exists yet. winner_status=not_evaluated means agents must not use it as routing proof. routes=[] means no comparable proven routes are recorded as benchmark evidence, even when verified/unproven mappings are tracked separately.', { $ref: '#/components/schemas/BenchmarkDetailResponse' }, { benchmark_id: 'finance-data-token-search', winner_claimed: false, benchmark_recorded: false, winner_status: 'not_evaluated', routes: [] }));
