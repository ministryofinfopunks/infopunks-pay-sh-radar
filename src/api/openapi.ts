...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/rh-chain/market/attention-quality/{contract}', rhChain('Get snapshot-history attention quality', 'Returns history-gated attention state and score. Price movement is not a main score input; paid visibility is context only.', 'RhChainAttentionQualityHistoryPayload', { ...rhExample, contract: '0xexample', snapshot_count: 0, state: 'source_required', score: null }, [pathParam('contract', 'Exact token contract address.') ]));
  ...
  add('get', '/v1/rh-chain/market-structure/attention-quality', rhChain('Get Paid Attention Watch', 'Returns DEX Screener boost and paid-order observations separately from history-gated attention quality. Paid visibility is not misconduct or organic conviction.', 'RhChainAttentionQualityPayload', { ...rhExample, title: 'Paid Attention Watch', observations: [], attention_quality: [] }));
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  return {
    ...
    RhChainAttentionQualityHistoryPayload: objectSchema({ contract: stringSchema(), snapshot_count: { type: 'integer' }, state: stringSchema(), score: { anyOf: [{ type: 'number' }, { type: 'null' }] }, components: freeformObject(), caveats: arrayOf(stringSchema()) }, ['contract', 'snapshot_count', 'state', 'score', 'components', 'caveats']),
    ...
    RhChainAttentionQualityPayload: objectSchema({ title: { const: 'Paid Attention Watch' }, observations: arrayOf(freeformObject()), attention_quality: arrayOf(freeformObject()), observed_at: dateTimeSchema() }, ['title', 'observations', 'attention_quality', 'observed_at']),
