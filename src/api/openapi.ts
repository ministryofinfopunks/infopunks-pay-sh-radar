...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/reflexive/pairs', rh4663('List verified Stock-quoted mission pairs', 'Lists only pairs whose quote contract matches the canonical Robinhood chain-4663 asset registry; ticker matches alone are rejected.', 'ReflexivePairList', { pairs: [], methodology_version: 'rmm-v0.3.0' }));
  add('get', '/v1/4663/reflexive/pairs/{id}', rh4663('Get mission-pair dossier', 'Returns immutable PositionManager-derived canonical locked-position principal when the NFT PoolKey and PAIR locker ownership verify. PoolManager balances are never used; unavailable values include a reason.', 'ReflexivePairDossier', { pair: { canonicality: 'verified' }, inventory: { scope: 'CANONICAL_LOCKED_POSITION', method: 'VERIFIED_LOCKED_POSITION_RECONSTRUCTION_V1', status: 'UNAVAILABLE', reason: 'POSITION_UNRESOLVED' } }, undefined, [pathParam('id', 'Deterministic mission-pair identifier.')]));
  add('get', '/v1/4663/reflexive/stocks/{symbol}', rh4663('Get Stock Token quote-economy dossier', 'Returns canonical token, tracked PAIR locked-position inventory (not ecosystem total), supply events, and separate gross-volume/inventory concepts. Net quote demand is unavailable until attributable.', 'ReflexiveStockDossier', { asset: { ticker: 'NVDA' }, tracked_position_inventory: null, tracked_absorption_pct: null, net_quote_demand: 'NOT_YET_ATTRIBUTABLE' }, undefined, [pathParam('symbol', 'Canonical Stock Token ticker.')]));
