...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/rh-chain/classifications', rhChain('List approved RH Chain classifications', 'Feature-flagged, read-only authoritative memory. Returns only active approved records keyed by exact contract; reviewer identity, internal audit notes, and manual-override rationale are never exposed. Provider observations cannot enter this response without a separate authenticated human approval.', 'RhChainPublicClassificationListPayload', { classifications: [{ chain: 'robinhood', contract: '0x1111111111111111111111111111111111111111', primary_layer: 'infrastructure', secondary_layers: ['agent'], confidence: 'high', review_status: 'approved', classification_version: 2, effective_at: '2026-07-19T00:00:00.000Z' }], page: 1, page_size: 25, has_more: false, authoritative: true, doctrine: 'Exact contract identity and reviewed memory outrank provider context.' }, [{ name: 'page', in: 'query', required: false, schema: { type: 'integer', minimum: 1, maximum: 10000 } }, { name: 'page_size', in: 'query', required: false, schema: { type: 'integer', minimum: 1, maximum: 100 } }], undefined, { '400': rhChainErrorResponse('invalid_request'), '404': rhChainErrorResponse('not_found'), '503': rhChainErrorResponse('rh_chain_classification_storage_unavailable') }));
  add('get', '/v1/rh-chain/projects', rhChain('List RH Chain project claim records', 'Feature-flagged bounded directory. Exact contracts establish identity; slugs are presentation only. This is not an investment ranking.', 'RhChainProjectDirectoryPayload', { items: [], next_cursor: null }, [{ name: 'cursor', in: 'query', required: false, schema: stringSchema() }, { name: 'limit', in: 'query', required: false, schema: { type: 'integer', minimum: 1, maximum: 50 } }]));
  add('get', '/v1/rh-chain/projects/{project_id}', rhChain('Get contract-first project claim page', 'Separates project-submitted claims, Infopunks observations, on-chain evidence, reviewed classification, published verdicts, unknowns, disputes, and receipts. Project submission never establishes reviewed truth.', 'RhChainProjectPayload', { project: { project_id: 'rhproj_example', primary_contract: '0x1111111111111111111111111111111111111111' }, project_submitted_claims: [], infopunks_observations: [], onchain_evidence: [], published_verdicts: [], receipts: [], not_financial_advice: true }, [pathParam('project_id', 'Stable project identifier. Exact contract remains the identity anchor.')]));
  add('get', '/v1/rh-chain/projects/{project_id}/claims', rhChain('List project-submitted claims', 'Returns project-submitted claims separately; they are never promoted to reviewed classifications by this endpoint.', 'RhChainProjectClaimsPayload', { project_id: 'rhproj_example', project_submitted_claims: [], not_financial_advice: true }, [pathParam('project_id', 'Project identifier.')]));
  add('get', '/v1/rh-chain/projects/{project_id}/observations', rhChain('List Infopunks observations', 'Returns independent observations and on-chain evidence without flattening them into project claims.', 'RhChainProjectObservationsPayload', { project_id: 'rhproj_example', infopunks_observations: [], onchain_evidence: [], not_financial_advice: true }, [pathParam('project_id', 'Project identifier.')]));
  add('get', '/v1/rh-chain/projects/{project_id}/receipts', rhChain('List project Intelligence Receipts', 'Returns published project receipts only. Attention Quality receipts retain their own semantics.', 'RhChainProjectReceiptsPayload', { project_id: 'rhproj_example', receipts: [], not_financial_advice: true }, [pathParam('project_id', 'Project identifier.')]));
  add('post', '/v1/rh-chain/projects/claims', rhChain('Queue a project claim', 'Public intake creates queued review only. It never verifies identity, approves a claim, publishes a verdict, or creates a reviewed classification.', 'RhChainProjectSubmissionResponse', { status: 'queued_for_review', no_endorsement: true, no_automatic_publication: true, no_automatic_classification: true, submission_id: 'rhclaim_example' }, undefined, jsonRequest({ $ref: '#/components/schemas/RhChainProjectClaimSubmissionRequest' }, { project_name: 'Example', chain: 'robinhood', exact_contract: '0x1111111111111111111111111111111111111111', contact: 'team@example.com', claim_category: 'product_live', claim_statement: 'The product is live.', supporting_evidence: ['https://example.com/docs'], disclosure_accepted: true, authority_to_submit_confirmed: true })));
  add('get', '/v1/rh-chain/intelligence-receipts', rhChain('List published project Intelligence Receipts', 'Lists immutable, content-addressed published project receipts. Corrections are represented by superseding receipts, never silent edits.', 'RhChainIntelligenceReceiptListPayload', { receipts: [], not_financial_advice: true }));
  add('get', '/v1/rh-chain/intelligence-receipts/{receipt_id}', rhChain('Get immutable Intelligence Receipt', 'Returns a published or superseded receipt with integrity, correction linkage, and a deterministic public-only sharing projection. Drafts and reviewer material are never returned.', 'RhChainIntelligenceReceiptPayload', { receipt: { receipt_id: 'rhir_example', integrity_hash: 'sha256:example', not_financial_advice: true }, share: { schema_version: 'rh_chain_share.v1', canonical_url: 'https://radar.infopunks.fun/rh-chain-signal-desk/intelligence-receipts/rhir_example', not_financial_advice: true }, not_financial_advice: true }, [pathParam('receipt_id', 'Immutable receipt identifier.')]));
  add('get', '/v1/rh-chain/market/provider-status', rhChain('Get DEX Screener provider status', 'Returns DEX Screener enablement, rolling health, latest latency, cache status, freshness, last success/failure, and fallback mode without exposing raw upstream errors. Provider observations are context only, never endorsement or approved signals.', 'RhChainMarketProviderStatusPayload', { ...rhExample, provider: 'dexscreener', enabled: false, fallback_mode: true, health: { state: 'disabled', activeCacheStatus: 'disabled' } }));
  add('get', '/v1/rh-chain/market', rhChain('Get Robinhood Chain Market Pulse', 'Returns a deterministic rolling-24-hour market-structure read: tracked volume, liquidity, transactions, token activity, reviewed layer composition, momentum, concentration, interpretation rules, provider health, freshness, warnings, and confidence. It is a bounded tracked-set view, not a complete chain index or token screener.', 'RhChainMarketPulsePayload', { ...rhExample, title: 'Robinhood Chain Market Pulse', observation_window: { label: 'Rolling 24 hours' }, metrics: {}, layer_composition: [], momentum: {}, concentration: {}, interpretation: { headline: 'Market structure is waiting for fresh pair evidence.', method: 'deterministic_rules_v1' }, provider_provenance: { provider: 'dexscreener', chain_id: 'robinhood' }, captured_at: '2026-07-19T00:00:00.000Z', freshness: 'unavailable', warnings: [], confidence: 'low' }));
  add('get', '/v1/rh-chain/market/tokens', rhChain('Refresh known RH Chain market context', 'Refreshes up to 300 known exact-contract tokens in provider batches of at most 30 through the validated, timeout-bounded adapter. Fresh, stale-while-revalidate, and bounded stale-if-error cache provenance is returned with normalized pairs. A provider failure fails soft and does not affect reviewed receipts or classifications.', 'RhChainMarketTokenListPayload', { ...rhExample, tokens: [], status: { provider: 'dexscreener', fallback_mode: true } }, [{ name: 'contracts', in: 'query', required: false, schema: stringSchema(), description: 'Optional comma-separated exact contracts, limited to 300 unique valid contracts. Tickers are not accepted as identity.' }], undefined, { '400': rhChainErrorResponse('rh_chain_market_contract_limit_exceeded') }));
  add('get', '/v1/rh-chain/market/tokens/{contract}', rhChain('Get exact-contract market context', 'Returns normalized pair observations, source links, raw-data version, cache provenance, freshness, confidence, and provider timestamps separately from Infopunks analysis and reviewed classification. Paid visibility is context only.', 'RhChainMarketTokenPayload', { ...rhExample, token: { contract: '0xexample' }, attention: { attention_state: 'source_required' }, classification: { source: 'review_required' } }, [pathParam('contract', 'Exact token contract address. Tickers never establish identity.')]));
  add('get', '/v1/rh-chain/market/boosts', rhChain('Get paid-attention context', 'Returns latest and top DEX Screener boost observations. Boosts are not misconduct and do not establish organic conviction.', 'RhChainMarketBoostsPayload', { ...rhExample, boosts: [], top_boosts: [], caveats: ['Paid-attention context only.'] }));
  add('get', '/v1/rh-chain/market/attention', rhChain('Get exact-contract attention context', 'Returns history-gated attention context. Without before/during/after snapshots the state is source_required; no attention quality score is inferred.', 'RhChainAttentionPayload', { ...rhExample, attention: { attention_state: 'source_required' } }, [{ name: 'contract', in: 'query', required: false, schema: stringSchema(), description: 'Exact token contract address.' }]));
  add('post', '/v1/rh-chain/market/snapshots/capture', rhChain('Capture RH Chain market snapshots', 'Admin-gated, low-frequency watchlist capture using the documented DEX Screener batch API. Ingestion and historical writes are independently feature flagged; legacy RH Chain automation remains backward compatible. Writes use deterministic snapshot identities and database conflict protection.', 'RhChainMarketSnapshotCapturePayload', { ...rhExample, captured: [], status: 'disabled', storage: { enabled: false, adapter: 'memory', durable: false, written: 0 } }, undefined, undefined, { '401': rhChainErrorResponse('admin_token_required'), '503': rhChainErrorResponse('rh_chain_snapshot_capture_disabled') }));
  add('get', '/v1/rh-chain/market/snapshots/{contract}', rhChain('List exact-contract market snapshots', 'Returns retained low-frequency market observations. Snapshot history is context only and cannot alter reviewed classifications or receipts.', 'RhChainMarketSnapshotListPayload', { ...rhExample, token: { contract: '0xexample' }, snapshots: [], snapshot_count: 0 }, [pathParam('contract', 'Exact token contract address.'), { name: 'window', in: 'query', required: false, schema: stringSchema(), description: 'Optional retained window such as 24h or 7d.' }]));
  add('get', '/v1/rh-chain/market/attention-quality/{contract}', rhChain('Get snapshot-history attention quality', 'When RH_CHAIN_ATTENTION_QUALITY_V2_ENABLED is false, returns the established history summary. When enabled, returns deterministic persisted-memory Attention Quality assessment components, windows, score gates, and zero provider requests in path. A numeric score appears only in measurable state.', 'RhChainAttentionQualityHistoryPayload', { ...rhExample, contract: '0xexample', snapshot_count: 0, state: 'source_required', score: null, assessment_state: 'insufficient_data', attention_quality_score: null, provider_requests_in_path: 0 }, [pathParam('contract', 'Exact token contract address.'), { name: 'window', in: 'query', required: false, schema: { type: 'string', enum: ['24h', '7d', '30d'] }, description: 'Persisted observation window for Attention Quality v2.' } ]));
  add('get', '/v1/rh-chain/market-structure', rhChain('Get 4663 Market Structure', 'Returns canonical-pair market composition, layer leaders, coverage, and trusted chain context. Layer percentages are classifier estimates, not complete chain accounting.', 'RhChainMarketStructurePayload', { ...rhExample, title: '4663 Market Structure', market_pulse: { layer_volume_estimates: [] }, market_composition: [] }));
  add('get', '/v1/rh-chain/market-structure/cross-layer', rhChain('Get RH Chain cross-layer assets', 'When durable reviewed classifications are enabled, returns bounded exact-contract intersections resolved with curated-memory precedence and enriched only from persisted market snapshots. Provider observations cannot create, promote, or approve an entry, and no provider request occurs in this request path. When the flag is disabled, the established Cross-Layer response remains unchanged.', 'RhChainCrossLayerPayload', { ...rhExample, title: 'Cross-Layer Assets', integration_enabled: true, headline: 'RWA × DeFi is the clearest reviewed intersection in the bounded set.', entries: [], captured_at: '2026-07-19T00:00:00.000Z', classification_coverage: { reviewed_exact_contracts: 1, cross_layer_eligible_exact_contracts: 1, percentage: 100 }, market_data_coverage: { eligible_exact_contracts: 1, with_persisted_market_data: 1, percentage: 100, provider_requests_in_path: 0 }, freshness: 'fresh', confidence: 'high', warnings: [], provider_provenance: { provider: 'dexscreener', external_requests_in_path: false }, classification_provenance: { precedence: ['curated_reviewed_memory', 'durable_approved_reviewed_classification', 'provider_context', 'unknown'], exact_contract_only: true }, bounded_universe: 'This response covers a bounded reviewed set and is not complete Robinhood Chain accounting.' }));
  add('get', '/v1/rh-chain/market-structure/attention-quality', rhChain('Get Paid Attention Watch', 'When Attention Quality v2 is enabled, returns a bounded reviewed exact-contract list derived only from persisted snapshots and reviewed memory; no provider request occurs in path. Paid visibility is not misconduct or organic conviction.', 'RhChainAttentionQualityPayload', { ...rhExample, title: 'Attention Quality', observations: [], attention_quality: [], provider_requests_in_path: 0, methodology_version: 'rh_chain_attention_quality_v2' }));
  add('get', '/v1/rh-chain/attention-quality/receipts/{receipt_id}', rhChain('Get published Attention Quality receipt', 'Returns an immutable published Attention Quality receipt. Draft and rejected receipts are never public.', 'RhChainAttentionQualityReceiptPayload', { ...rhExample, receipt_id: 'rhaq_example', status: 'published', schema_version: 1 }, [pathParam('receipt_id', 'Immutable Attention Quality receipt identifier.')]));
  const reviewPipelineBody = jsonRequest(freeformObject(), { market_structure_layer: 'unknown', secondary_tags: ['source_required_for_claims'], reviewer_note: 'Human review retained source gaps.', missing_evidence: ['Primary source'], caveats: ['Provider context only.'] });
  const reviewPipelineContract = [pathParam('contract', 'Exact 0x token contract. Tickers are never accepted as identity.')];
  add('get', '/v1/rh-chain/review-pipeline', rhChain('Get Review Pipeline', 'Returns exact-contract discoveries and Signal Desk review memory. Provider-only records start source_required; reviewed memory outranks provider context.', 'RhChainReviewPipelinePayload', { ...rhExample, title: 'RH Chain Signal Desk Review Pipeline', items: [{ contract: '0x1111111111111111111111111111111111111111', review_state: 'source_required', reviewer_attribution: 'Signal Desk' }] }, [{ name: 'day', in: 'query', required: false, schema: stringSchema(), description: 'Optional UTC daily-review date.' }]));
  add('post', '/v1/rh-chain/review-pipeline/start-daily-review', rhChain('Start Daily Review', 'Creates or resumes an unpublished daily review surface. It does not approve a token or publish a Daily Receipt.', 'RhChainDailyReviewStartPayload', { ...rhExample, day: '2026-07-17', status: 'unpublished' }, undefined, jsonRequest(freeformObject(), { day: '2026-07-17' })));
  add('post', '/v1/rh-chain/review-pipeline/{contract}/classify', rhChain('Classify Review Record', 'Sets a human-review classification and secondary tags for an exact-contract record.', 'RhChainReviewRecordPayload', { ...rhExample, item: { contract: '0x1111111111111111111111111111111111111111', market_structure_layer: 'infrastructure', review_state: 'source_required' } }, reviewPipelineContract, reviewPipelineBody, { '400': rhChainErrorResponse('exact_contract_required') }));
  add('post', '/v1/rh-chain/review-pipeline/{contract}/promote-to-market-structure', rhChain('Promote to Market Structure Candidate', 'Creates a reviewed-intake candidate with missing-evidence and caveat memory. It never creates an approved signal.', 'RhChainReviewRecordPayload', { ...rhExample, item: { contract: '0x1111111111111111111111111111111111111111', review_state: 'promoted_to_market_structure' } }, reviewPipelineContract, reviewPipelineBody, { '400': rhChainErrorResponse('exact_contract_required') }));
  add('post', '/v1/rh-chain/review-pipeline/{contract}/promote-to-100-receipts', rhChain('Promote to 100 Receipts Candidate', 'Creates a candidate only; it never completes a receipt.', 'RhChainReviewRecordPayload', { ...rhExample, item: { contract: '0x1111111111111111111111111111111111111111', review_state: 'promoted_to_100_receipts_candidate' } }, reviewPipelineContract, reviewPipelineBody, { '400': rhChainErrorResponse('exact_contract_required') }));
  add('post', '/v1/rh-chain/review-pipeline/{contract}/add-to-daily-draft', rhChain('Add Daily Receipt Draft Context', 'Adds exact-contract review context to an unpublished Daily Receipt draft.', 'RhChainDailyReviewDraftPayload', { ...rhExample, daily_receipt_draft: { day: '2026-07-17', status: 'unpublished' } }, reviewPipelineContract, reviewPipelineBody, { '400': rhChainErrorResponse('exact_contract_required') }));
  add('post', '/v1/rh-chain/review-pipeline/{contract}/set-outcome-check', rhChain('Schedule Outcome Check', 'Schedules an outcome check seven days after review unless a valid time is supplied.', 'RhChainReviewRecordPayload', { ...rhExample, item: { contract: '0x1111111111111111111111111111111111111111', review_state: 'outcome_check_scheduled' } }, reviewPipelineContract, reviewPipelineBody, { '400': rhChainErrorResponse('exact_contract_required') }));
  add('get', '/v1/rh-chain/review-pipeline/daily-summary', rhChain('Get Daily Review Summary', 'Returns review state counts, attention context, cross-layer candidates, duplicate ticker warnings, and a source-limited Daily Receipt headline.', 'RhChainDailyReviewSummaryPayload', { ...rhExample, reviewed_count: 0, source_required_count: 0, paid_attention_detected: false }, [{ name: 'day', in: 'query', required: false, schema: stringSchema(), description: 'Optional UTC daily-review date.' }]));
  ...
  const rh4663 = (summary: string, description: string, schema: string, example: unknown, requestBody?: JsonSchema, parameters?: unknown[]) => ({
    tags: ['Infopunks //4663'], summary, description,
    ...(parameters ? { parameters } : {}), ...(requestBody ? { requestBody } : {}),
    responses: envelopedResponses({ $ref: `#/components/schemas/${schema}` }, example)
  });
  add('get', '/v1/4663', rh4663('Get the 4663 operating snapshot', 'Returns persisted chain rotation memory, Pulse consensus, Today, Signal Hunt, Genesis progress, and explicit receipt semantics. No external provider is called on this read path.', 'Rh4663Overview', { identity: 'INFOPUNKS // 4663', thesis: 'WE WATCH THE FLOW.' }));
  add('get', '/v1/4663/frontdoor', rh4663('Get the 4663 Front Door read model', 'Returns the compact, public, anonymous, request-identity-agnostic global snapshot for /4663. It references canonical Census, Watch, Preflight, Signal, and Pulse objects but is not canonical evidence itself. It never contains wallet-specific or private user state; user overlays belong under /v1/4663/me/*. `frontdoor_version` changes only when the visible derived source state changes, so clients can cheaply revalidate without diffing the full snapshot. It uses latest persisted reads only, exposes per-source health, and may return partial state when a module is unavailable.', 'Rh4663FrontdoorState', { object_type: 'RH_4663_FRONTDOOR_STATE', generated_at: '2026-09-03T12:00:00.000Z', frontdoor_version: { object_type: 'FRONTDOOR_VERSION', version: 1821, changed: ['RMM_CENSUS', 'AI_NVDA_CASE', 'PULSE'], generated_at: '2026-09-03T12:00:00.000Z' }, frontdoor_version_durability: 'PERSISTENT', now_cards: [], watch_cards: [], open_loops: [], system_status: { state: 'partial', source_health: { census: { status: 'available' } } } }));
  add('get', '/v1/4663/share/{shareObjectId}', rh4663('Get a public //4663 share object', 'Projects one public, source-bound evidence object into a cacheable social contract. It never reads private follows, drafts, balances, portfolio data, or personalised state. Immutable receipts, resolutions, checkpoints, Census, Radar, Shadow, and change objects receive immutable cache policy; current NOW/WATCH/OPEN LOOP objects are short-cache versioned snapshots.', 'Rh4663ShareObject', { share_object_id: 'rmm_census_observation:census-1', share_type: 'RMM_CENSUS_OBSERVATION', primary_statement: 'Radar verified direct markets.', evidence_state: 'VERIFIED', share_version: 'rh4663.share.v1', privacy_state: 'PUBLIC' }, undefined, [pathParam('shareObjectId', 'Registry identity, for example now_finding:card-1.') ]));
  add('get', '/v1/4663/me/call', rh4663('Get the current user CALL state', 'Returns the minimal wallet-scoped CALL overlay for the current fixed UTC window. Supply the wallet identity in the `wallet` query parameter; an absent identity returns a safe anonymous/no-call state. Responses are private and never publicly cacheable. This read model does not alter canonical CALL or RESOLUTION receipts.', 'Rh4663MyCall', { authenticated: false, window_id: 'rh4663:2026-09-03', has_called: false, call_receipt_reference: null, selection: null, confidence: null, submitted_at: null, resolution_state: null, resolution_receipt: null, my_4663_version: '0' }, undefined, [{ name: 'wallet', in: 'query', required: false, schema: { type: 'string', pattern: '^0x[0-9a-fA-F]{40}$' }, description: 'Connected wallet address for the private overlay.' }]));
  add('get', '/v1/4663/me/proof', rh4663('Get the current user PROOF_PROFILE', 'Returns the private identity-scoped proof overlay. It is derived from canonical CALL and published RESOLUTION receipts, excludes unresolved calls from accuracy, and contains no wallet portfolio, balance, trading history, rewards, points, or opaque score. Responses are private and never publicly cacheable.', 'Rh4663MyProof', { authenticated: false, profile: null, my_4663_version: '0' }, undefined, [{ name: 'wallet', in: 'query', required: false, schema: { type: 'string', pattern: '^0x[0-9a-fA-F]{40}$' }, description: 'Connected wallet address for the private overlay.' }]));
  add('get', '/v1/4663/me/changes', rh4663('Get private return changes', 'Returns a private, no-store return overlay: the latest resolved CALL, personal resolution event, and context changes observed after a pending CALL. It never returns balances, portfolio state, or trading activity; global changes remain anonymous and cacheable on the public Front Door.', 'Rh4663MyChanges', { authenticated: false, resolved_call: null, personal_events: [], pending_call: null, my_4663_version: '0' }, undefined, [{ name: 'wallet', in: 'query', required: false, schema: { type: 'string', pattern: '^0x[0-9a-fA-F]{40}$' }, description: 'Connected wallet address for the private overlay.' }]));
  add('get', '/v1/4663/me', rh4663('Get the MY 4663 local-first relevance overlay', 'Accepts a bounded browser-local list of stable follows and returns only references into the global Front Door. This response is private, no-store, and never changes the public Front Door or its ETag.', 'My4663State', { object_type: 'MY_4663_STATE', follows: [], changed_followed_subjects: [], followed_open_loops: [], followed_now_items: [], personal_call_state: null, personal_proof: null, last_seen_my4663_event_id: null, my_4663_version: '0' }, undefined, [{ name: 'follows', in: 'query', required: false, schema: { type: 'string', maxLength: 12000 }, description: 'JSON encoded local follows; never persisted by this endpoint.' }, { name: 'last_seen_my4663_event_id', in: 'query', required: false, schema: { type: 'string', maxLength: 160 }, description: 'Separate personal overlay cursor.' }]));
  add('get', '/v1/4663/prints/latest', rh4663('Get the latest 4663 PRINT', 'Returns the latest published market-state evidence object. PRINT is evidence context for a CALL, never a Protocol Receipt; every metric carries source, observation window, methodology, freshness, and confidence.', 'Rh4663Print', { print_id: '0830', receipt_kind: 'MARKET_STATE_EVIDENCE', regime: 'SPECULATIVE EXPANSION' }));
  add('get', '/v1/4663/prints', rh4663('List frozen 4663 PRINTs', 'Lists frozen market-memory PRINTs and the immutable Aug 30 campaign snapshot. Review candidates are excluded.', 'Rh4663PrintList', { prints: [{ print_id: 'rh-print-2026-08-30', status: 'published' }] }));
  add('get', '/v1/4663/prints/{printId}', rh4663('Get a 4663 PRINT', 'Returns a canonical market-state evidence object by print identifier. It preserves reporting windows and ambiguity instead of collapsing unlike measures into an ATH claim.', 'Rh4663Print', { print_id: '0830', title: 'ROBINHOOD CHAIN IS RUNNING HOT' }, undefined, [pathParam('printId', 'Canonical PRINT identifier, for example 0830.')]));
  add('get', '/v1/4663/prints/{printId}/share', rh4663('Get a 4663 PRINT share object', 'Returns canonical PRINT URLs and landscape, square, and portrait image routes. The visual is a share artifact, not a Protocol Receipt.', 'Rh4663PrintShareObject', { object_type: 'market_state_print', print_id: 'rh-print-2026-08-30' }, undefined, [pathParam('printId', 'Canonical PRINT identifier.')]));
  add('get', '/v1/4663/print-candidate', rh4663('Generate a 4663 PRINT candidate', 'Normalizes existing cached provider observations into a review-only candidate. It shows source windows, gaps, freshness, and disagreements; it never publishes a PRINT.', 'Rh4663PrintCandidate', { candidate_id: 'rh-print-candidate-2026-08-31', lifecycle: 'CANDIDATE', warnings: ['MISSING_FINAL_UTC_TRANSACTIONS'] }, undefined, [{ name: 'date', in: 'query', required: false, schema: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' } }]));
  add('get', '/v1/4663/observations', rh4663('Get final 4663 UTC-day observations', 'Returns only persisted completed UTC-calendar-day chain transactions and chain-wide DEX-volume observations. Rolling 24-hour context is intentionally excluded. Missing observations remain null; zero appears only when the provider verified zero.', 'Rh4663UtcDayObservations', { date: '2026-08-31', transactions: { observation_id: 'rh4663:transactions:2026-08-31:blockscout:rh4663.utc-day-observation.v1', value: 5521213, window_type: 'UTC_CALENDAR_DAY', freshness: 'HISTORICAL_FINAL' }, dex_volume: { observation_id: 'rh4663:dex_volume_usd:2026-08-31:defillama:rh4663.utc-day-observation.v1', value: 874800000, window_type: 'UTC_CALENDAR_DAY', freshness: 'HISTORICAL_FINAL' }, status: 'FINALIZED', warnings: [] }, undefined, [{ name: 'date', in: 'query', required: true, schema: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' } }]));
  add('post', '/internal/4663/observations/utc-day/{date}/refresh', rh4663('Refresh completed 4663 UTC-day observations', 'Reviewer-gated, idempotent ingestion of completed-day Blockscout transaction and DefiLlama daily DEX-volume observations. It retains revisions instead of overwriting accepted source history and never publishes or freezes a PRINT.', 'Rh4663UtcDayRefresh', { date: '2026-08-31', status: 'FINALIZED', warnings: [] }, undefined, [pathParam('date', 'Completed UTC date in YYYY-MM-DD format.')]));
  add('post', '/v1/4663/campaign/events', rh4663('Record private 4663 funnel telemetry', 'Accepts one allowlisted, privacy-preserving campaign event. Wallets, signatures, and free-form values are rejected.', 'Rh4663CampaignTelemetry', { accepted: true }, jsonRequest(objectSchema({ event: enumSchema(['4663_print_viewed', '4663_make_call_clicked', '4663_call_accepted', '4663_consensus_viewed', 'call_card_viewed', 'call_option_selected', 'call_confidence_changed', 'call_review_opened', 'call_sign_started', 'call_sign_succeeded', 'call_sign_failed', 'call_receipt_opened', 'call_shared', 'call_resolution_viewed', 'call_again_started', 'open_loop_viewed', 'open_loop_opened', 'open_loop_source_opened', 'return_change_summary_viewed', 'return_change_opened', 'pending_call_changes_viewed', 'resolved_call_return_viewed', 'frontdoor_return_visit']), surface: enumSchema(['print', 'pulse', 'call', 'consensus', 'resolution', 'home']), print_id: stringSchema(), window_id: stringSchema() }, ['event']), { event: 'call_card_viewed', surface: 'home', window_id: 'rh4663:2026-09-03' })));
  add('get', '/v1/4663/pulse', rh4663('Get RH Pulse', 'Returns the fixed UTC window, deterministic consensus, options, and locked mechanics version.', 'Rh4663Pulse', { window: { window_id: 'rh4663:2026-08-13' }, consensus: { total_calls: 0, state: 'unavailable' } }));
  add('post', '/v1/4663/pulse/payload', rh4663('Build canonical Pulse payload', 'Builds deterministic canonical serialization for wallet signing. It does not create a call or receipt.', 'Rh4663CanonicalPayload', { canonical_serialization: '{"version":"infopunks.rh-pulse.call.v1"}', payload_hash: '0x…' }, jsonRequest({ $ref: '#/components/schemas/Rh4663PulsePayloadRequest' }, { wallet: '0x1111111111111111111111111111111111111111', rotation: 'MEMES', confidence: 72 })));
  add('post', '/v1/4663/pulse/calls', rh4663('Create immutable CALL RECEIPT', 'Verifies the EVM personal-sign signature and atomically enforces one call per wallet per fixed UTC window. Existing receipts are never updated.', 'Rh4663CallReceipt', { receipt_kind: 'PROTOCOL_RECEIPT', protocol_receipt_type: 'CALL', immutable: true }, jsonRequest({ $ref: '#/components/schemas/Rh4663PulseCallRequest' }, { wallet: '0x1111111111111111111111111111111111111111', rotation: 'MEMES', confidence: 72, signature: '0x…' })));
  add('get', '/v1/4663/pulse/windows/{windowId}', rh4663('Get Pulse window', 'Returns the explicit open, closed, resolved, or published state; deterministic Phase 1 consensus with counts and percentages; and any persisted acceptance commitment and anchor state. Anchor submitting, submitted, confirmed, and failed are distinct. Consensus describes participant calls, never the resolved market outcome.', 'Rh4663Window', { state: 'published', consensus: { total_calls: 1284 }, acceptance: { root: '0x…' } }, undefined, [pathParam('windowId', 'Pulse window identifier, for example rh4663:2026-08-13.')]));
  add('get', '/v1/4663/pulse/windows/{windowId}/resolution', rh4663('Get published Pulse resolution', 'Returns only a published deterministic resolution. It identifies the frozen observation window, accepted inputs, dependency hashes, tie handling, degraded sources, NO QUALIFIED ROTATION behavior, acceptance root, and anchor state.', 'Rh4663WindowResolution', { window_id: 'rh4663:2026-08-13', state: 'published', resolved_category: 'STOCK_TOKENS' }, undefined, [pathParam('windowId', 'Pulse window identifier.')]));
  add('get', '/v1/4663/pulse/receipts/{receiptId}/proof', rh4663('Get CALL inclusion proof', 'Returns a deterministic Merkle inclusion proof that allows a verifier to prove the CALL payload hash belongs to the frozen window acceptance root without trusting the application database. Anchor submitted and confirmed are distinct states.', 'Rh4663MerkleProof', { receipt_id: 'call_example', verified: true, acceptance_root: '0x…', anchor: { state: 'confirmed' } }, undefined, [pathParam('receiptId', 'Immutable CALL receipt identifier.')]));
  add('get', '/v1/4663/pulse/receipts/{receiptId}/share', rh4663('Get deterministic Pulse share object', 'Returns one reusable presentation object for pending, correctly resolved, or incorrectly resolved calls, with canonical proof links and landscape, square, and portrait renderer URLs. The visual is not itself a protocol receipt.', 'Rh4663ShareObject', { object_type: 'resolved_correct_call', proof_url: '/4663/proof/call_example' }, undefined, [pathParam('receiptId', 'Immutable CALL receipt identifier.')]));
  add('get', '/v1/4663/pulse/windows/{windowId}/share', rh4663('Get deterministic Pulse window share object', 'Returns the same universal share model for participant consensus or a published window result. It keeps consensus and deterministic resolution explicit and distinct.', 'Rh4663WindowShareObject', { object_type: 'window_result', total_calls: 1284, consensus_category: 'STOCK_TOKENS', resolved_category: 'STOCK_TOKENS' }, undefined, [pathParam('windowId', 'Pulse window identifier.')]));
  add('get', '/v1/4663/pulse/reputation/{wallet}', rh4663('Get transparent Pulse reputation', 'Returns calls, resolved calls, correct calls, accuracy, current streak, and machine-readable evidence. Unresolved calls are excluded from accuracy and do not break a streak; an incorrect result resets it. No opaque score or balance weighting is used.', 'Rh4663Reputation', { calls: 32, resolved_calls: 28, correct_calls: 20, accuracy: 0.7143, current_streak: 4 }, undefined, [pathParam('wallet', 'Exact EVM wallet address.')]));
  add('get', '/v1/4663/proof/{wallet}', rh4663('Get a public PROOF_PROFILE', 'Returns a shareable receipt-backed judgment record for one wallet identity. It contains only CALL/RESOLUTION-derived statistics, category breakdowns, Genesis linkage, recent receipt links, and methodology. It is not a portfolio, balance, trading-history, reward, points, or opaque reputation endpoint.', 'Rh4663ProofProfile', { object_type: 'PROOF_PROFILE', calls: 38, resolved: 31, correct: 21, unresolved: 7, accuracy: 0.6774, high_confidence_accuracy: 0.72 }, undefined, [pathParam('wallet', 'Exact EVM wallet address represented by the public proof profile.')]));
  add('get', '/v1/4663/today', rh4663('Get Today on 4663', 'Returns the current durable daily edition with events, category flows, key signal, optional Pulse consensus, evidence references, confidence, source timestamps, and explicit provider state.', 'Rh4663TodayEdition', { edition_id: 'today_4663_20260813_v1', date: '2026-08-13', provider_state: 'degraded' }));
  add('get', '/v1/4663/today/archive', rh4663('List Today archive', 'Returns durable daily editions newest first.', 'Rh4663TodayArchive', { editions: [] }));
  add('get', '/v1/4663/today/{date}', rh4663('Get archived Today edition', 'Returns one immutable daily edition by UTC date.', 'Rh4663TodayEdition', { edition_id: 'today_4663_20260813_v1', date: '2026-08-13' }, undefined, [pathParam('date', 'UTC date in YYYY-MM-DD format.')]));
  add('get', '/v1/4663/signals', rh4663('List published 4663 Signal Cards', 'Returns immutable public Signal Cards from the shared event engine plus public-safe watch items. Filters include category, subject, signal_type, since, and the only public-safe publication_state: published. No provider is called on this read path. Signal Cards are presentation objects, not receipts.', 'Rh4663SignalList', { signals: [], watching: [], provider_requests_in_path: 0, guarantee_notice: 'Signal Cards are editorial intelligence, not Evidence Receipts or Protocol Receipts.' }, undefined, [{ name: 'category', in: 'query', required: false, schema: stringSchema() }, { name: 'subject', in: 'query', required: false, schema: stringSchema() }, { name: 'signal_type', in: 'query', required: false, schema: stringSchema() }, { name: 'since', in: 'query', required: false, schema: dateTimeSchema() }, { name: 'publication_state', in: 'query', required: false, schema: { const: 'published' } }]));
  add('post', '/v1/4663/signals', rh4663('Submit a 4663 signal', 'Creates a low-friction public Signal Card while permanently retaining the original submitter attribution.', 'Rh4663Signal', { signal_id: 'sig4663_example', lifecycle_state: 'submitted', attribution_immutable: true }, jsonRequest({ $ref: '#/components/schemas/Rh4663SignalSubmissionRequest' }, { title: 'Wallet flow changed', category: 'wallet', thesis: 'A source-linked wallet flow deserves review.', source_url: 'https://example.com/source', submitter: '@scout' })));
  add('post', '/v1/4663/signals/{signalId}/evidence', rh4663('Add signal evidence', 'Adds a source reference without changing original submitter attribution.', 'Rh4663Signal', { signal_id: 'sig4663_example', lifecycle_state: 'evidence_added' }, jsonRequest({ $ref: '#/components/schemas/Rh4663SignalEvidenceRequest' }, { url: 'https://example.com/evidence', label: 'Primary evidence' }), [pathParam('signalId', '4663 signal identifier.')]));
  add('get', '/v1/4663/signals/{signalId}', rh4663('Inspect a Signal Card proof page object', 'Returns one immutable published Signal Card with evidence, significance and anomaly components, heuristic version, publication time, finder attribution, and additive correction history. Internal reviewer notes are excluded.', 'Published4663Signal', { signal_id: 'SIGNAL-4663-0084', representation_kind: 'SIGNAL_CARD', immutable: true, headline: 'NVDA VOLUME +184%', correction_state: 'ORIGINAL' }, undefined, [pathParam('signalId', 'Published Signal Card identifier.')]));
  add('get', '/v1/4663/signals/{signalId}/evidence', rh4663('Inspect Signal Card evidence', 'Returns the persisted evidence references that support the public Signal Card and any evidence attached by additive corrections. It never returns arbitrary provider payloads or reviewer notes.', 'Rh4663SignalEvidenceBundle', { signal_id: 'SIGNAL-4663-0084', evidence: [], traceability: 'Every public numeric fact is derived from persisted evidence.' }, undefined, [pathParam('signalId', 'Published Signal Card identifier.')]));
  add('get', '/v1/4663/lenses/{lens}', rh4663('Read a 4663 market lens', 'Returns a category view over the same published normalized-event and Signal publication system. Initial lenses cover meme, stock token, RWA DeFi, stablecoin, culture/NFT, and agent/utility activity. Lenses are views, not ingestion systems.', 'Rh4663MarketLens', { lens: 'STOCK_TOKENS', source: 'shared_normalized_4663_event_system', signals: [], provider_requests_in_path: 0 }, undefined, [pathParam('lens', 'Lens name or intelligence category.')]));
  add('get', '/v1/4663/rotation', rh4663('Get live market-rotation intelligence', 'Returns a versioned heuristic rotation lens derived from published Signal evidence. LIVE ROTATION SIGNAL is explicitly separate from RH Pulse protocol resolution and cannot overwrite it.', 'Rh4663RotationLens', { object_type: 'LIVE_ROTATION_SIGNAL', protocol_resolution: false, leader: 'STOCK_TOKENS' }));
  add('get', '/v1/4663/events', rh4663('List normalized 4663 events', 'Returns the reusable normalized event layer consumed by public 4663 features.', 'Rh4663EventList', { events: [] }));
  add('get', '/v1/4663/receipts', rh4663('List 4663 Protocol Receipts', 'Returns immutable protocol CALL receipts. This endpoint never labels Signal Cards as receipts.', 'Rh4663ReceiptList', { receipt_kind: 'PROTOCOL_RECEIPT', receipts: [], immutable: true }));
  add('get', '/v1/4663/receipts/{receiptId}', rh4663('Get a 4663 Protocol Receipt', 'Returns one canonical immutable CALL or published RESOLUTION receipt. A RESOLUTION receipt links to, and never changes, its CALL receipt.', 'Rh4663ProtocolReceipt', { receipt_id: 'IP-RES-8F31', receipt_kind: 'PROTOCOL_RECEIPT', protocol_receipt_type: 'RESOLUTION', immutable: true }, undefined, [pathParam('receiptId', 'Protocol receipt identifier.')]));
  add('get', '/v1/4663/reflexive', rh4663('Get Reflexive Radar state', 'Returns RMM market observations and thesis evidence. These objects are explicitly not CALL or RESOLUTION receipts; unavailable quantities remain unavailable.', 'ReflexiveRadarSnapshot', { assets: [], pairs: [], events: [], thesis: [] }));
  add('get', '/v1/4663/reflexive/watch', rh4663('Get Reflexive Markets Watch', 'Returns the public WATCH to RADAR evidence bridge. Watch claims are provenance-scoped discovery objects and Watch verdicts are WATCH_INTERPRETATION, never RADAR_VERDICT. The feed cannot create verified inventory, verified supply, verified pool provenance, thesis confirmation, or protocol receipts. AI/NVDA includes a tiny D7 re-audit object with precommitted H2B verdict logic.', 'ReflexiveMarketsWatch', { object_type: 'REFLEXIVE_MARKETS_WATCH', doctrine: ['WATCH_DISCOVERS', 'RADAR_VERIFIES'], feed: [], falsification_queue: [], thesis_board: [] }));
  add('get', '/v1/4663/reflexive/watch/cases', rh4663('List RWA Activation Cases', 'Returns the RWA Activation Casebook grouped as confirming, mixed, falsification, and unverified cases. This is not a leaderboard and no black-box score is emitted.', 'RwaActivationCaseList', { cases: [{ case_id: 'AI_NVDA_CAPITAL_VS_FLOW', current_evidence_state: 'MIXED' }], casebook: { mixed_cases: [] }, falsification_queue: [] }));
  add('get', '/v1/4663/reflexive/watch/cases/{id}', rh4663('Get RWA Activation Case', 'Returns one case with WHAT HAPPENED, Watch interpretation, claims, Radar evidence links, persistence, AI/NVDA D7 re-audit fields when applicable, falsification criteria, open gaps, thesis impact, and append-only state transitions. No trading CTA or token recommendation is exposed.', 'RwaActivationCaseDetail', { case: { case_id: 'AI_NVDA_CAPITAL_VS_FLOW', object_type: 'RWA_ACTIVATION_CASE', research_observations: [{ object_type: 'AI_NVDA_D7_RE_AUDIT', checkpoint: 'D7', h2b_verdict: 'OBSERVING', capital_regime: 'INSUFFICIENT_DATA', flow_regime: 'INSUFFICIENT_DATA', h2b_policy: { policy_version: 'H2B_D7_PRECOMMIT_V1' } }] }, claims: [], audit_targets: [] }, undefined, [pathParam('id', 'RWA Activation Case identifier.')]));
  add('get', '/v1/4663/reflexive/watch/claims', rh4663('List Reflexive Watch Claims', 'Returns immutable REFLEXIVE_WATCH_CLAIM objects with source provenance, claimed metrics, provisional interpretation, Watch verdict, and evidence status. If a source changes its figures a new claim is created.', 'ReflexiveWatchClaimList', { claims: [{ object_type: 'REFLEXIVE_WATCH_CLAIM', watch_verdict_type: 'WATCH_INTERPRETATION', immutable: true }] }));
  add('get', '/v1/4663/reflexive/census', rh4663('Get immutable RMM Category Census', 'Returns the latest immutable breadth census. It independently establishes the active canonical Stock Token denominator and counts only pairs with deterministic canonical Stock Token quote direction. Watch claims never qualify a market or confirm RA1.', 'RmmCategoryCensus', { object_type: 'RMM_CATEGORY_CENSUS', claimed_pair_count: 27, verified_pair_count: 0, rmm_penetration: { numerator: 0, denominator: 0, percentage: null }, category_evidence: { ra1_impact: 'BREADTH_SUPPORT_ONLY_NOT_CONFIRMED' } }));
  add('get', '/v1/4663/reflexive/census/pairs', rh4663('List RMM Census pair records', 'Returns claimed and unresolved records as well as verified markets. A displayed pair is not treated as a quote relationship without deterministic quote-direction evidence.', 'RmmCensusPairList', { pairs: [], census: null }));
  add('get', '/v1/4663/reflexive/census/stocks/{symbol}', rh4663('Get census records for one Stock Token', 'Returns one canonical-universe entry and its claimed RMM records. Identity is canonical asset id and contract, never ticker alone.', 'RmmCensusStock', { stock: { ticker: 'NVDA' }, pairs: [] }, undefined, [pathParam('symbol', 'Canonical Stock Token ticker.')]));
  add('get', '/v1/4663/reflexive/pairs', rh4663('List verified Stock-quoted mission pairs', 'Lists only pairs whose quote contract matches the canonical Robinhood chain-4663 asset registry; ticker matches alone are rejected.', 'ReflexivePairList', { pairs: [], methodology_version: 'rmm-v0.4.2' }));
  add('get', '/v1/4663/reflexive/pairs/{id}', rh4663('Get mission-pair dossier', 'Returns immutable PositionManager-derived principal only after the locked NFT PoolKey, owner, and StateView core-position liquidity all reconcile. PoolManager balances are never used; unavailable values include a reason.', 'ReflexivePairDossier', { pair: { canonicality: 'verified' }, inventory: { scope: 'CANONICAL_LOCKED_POSITION', method: 'VERIFIED_LOCKED_POSITION_RECONSTRUCTION_V1', status: 'UNAVAILABLE', reason: 'POSITION_STATE_MISMATCH' } }, undefined, [pathParam('id', 'Deterministic mission-pair identifier.')]));
  add('get', '/v1/4663/reflexive/stocks/{symbol}', rh4663('Get Stock Token quote-economy dossier', 'Returns canonical token and same-block TRACKED_PAIR_CANONICAL_LOCKED_POSITIONS aggregate (not ecosystem total), supply observations, and separate inventory concepts. Net quote demand is unavailable until attributable.', 'ReflexiveStockDossier', { asset: { ticker: 'NVDA' }, tracked_pair_locked_inventory: { status: 'INCOMPLETE', aggregation_scope: 'TRACKED_PAIR_CANONICAL_LOCKED_POSITIONS' }, net_quote_demand: 'NOT_YET_ATTRIBUTABLE' }, undefined, [pathParam('symbol', 'Canonical Stock Token ticker.')]));
  add('get', '/v1/4663/reflexive/stocks/PLTR/preflight', rh4663('Get immutable PLTR preflight state', 'Returns a persisted v0.4.5 PLTR_PREFLIGHT_STATE observation. `observation_id` may select an exact historical state; no field is refreshed during retrieval. Direct-market verification requires a canonical PoolManager Initialize event, a re-derived PoolId, StateView state and exact token identity.', 'PltrPreflightState', { observation_id: 'pltr-preflight-52371729-20260902064656000', immutable: true, state_type: 'PLTR_PREFLIGHT_STATE', readiness: { status: 'PARTIAL', missing_prerequisites: ['independently verified direct PLTR market state unavailable'] }, first_party_footprint: { schema: 'PROPOSED_FIRST_PARTY_PLTR_FOOTPRINT_V1', values_populated: false } }, undefined, [{ name: 'observation_id', in: 'query', required: false, schema: stringSchema(), description: 'Immutable preflight-state observation identifier.' }]));
  add('post', '/v1/4663/reflexive/preflight/ipx-pltr/simulate', rh4663('Simulate a hypothetical IPX/PLTR market', 'Consumes one explicit immutable READY_FOR_SIMULATION PLTR observation and either one hypothetical first-party IPX configuration or two-to-four comparable configurations for a comparison matrix. The pure v0.5.1 model performs concentrated-liquidity, buy/sell, supply, price, basis, session, capital/flow, route, and range stress without fetching live fields or exposing any transaction capability. Outputs are PREFLIGHT_SIMULATION_RECORD objects, never CALL or RESOLUTION receipts and never launch authorization.', 'IpxPltrPreflightSimulation', { record_type: 'PREFLIGHT_SIMULATION_RECORD', simulation_id: 'ipxsim-sha256', state_snapshot_id: 'pltr-preflight-52406504-20260902074509000', immutable: true, result: { synthetic_ipx_asset: { contract_address: null }, draft_simulation_verdict: { label: 'DRAFT_SIMULATION_VERDICT', verdict: 'DEGRADE', launch_authorization: false } } }, jsonRequest({ oneOf: [objectSchema({ state_snapshot_id: stringSchema(), hypothetical_configuration: freeformObject() }, ['state_snapshot_id', 'hypothetical_configuration']), objectSchema({ state_snapshot_id: stringSchema(), hypothetical_configurations: arrayOf(freeformObject()) }, ['state_snapshot_id', 'hypothetical_configurations'])] }, { state_snapshot_id: 'pltr-preflight-52406504-20260902074509000', hypothetical_configuration: { simulation_name: 'Illustrative anchor stress', architecture: 'PLTR_ANCHOR', asset_owner: 'INFOPUNKS', first_party_asset: true } })));
  add('get', '/v1/4663/reflexive/preflight/ipx-pltr/simulations/{id}', rh4663('Get an immutable IPX/PLTR preflight simulation', 'Returns one append-only PREFLIGHT_SIMULATION_RECORD by deterministic simulation id. It performs no provider fetch and exposes no trading or execution operation.', 'IpxPltrPreflightSimulation', { record_type: 'PREFLIGHT_SIMULATION_RECORD', simulation_id: 'ipxsim-sha256', immutable: true }, undefined, [pathParam('id', 'Deterministic IPX/PLTR simulation identifier.')]));
  add('get', '/v1/4663/reflexive/preflight/ipx-pltr/shadow/candidates', rh4663('List pinned shadow candidates', 'Returns immutable v0.5.1 SHADOW_ONLY candidate records. Candidates are longitudinal replay inputs, not launch parameters or recommendations.', 'IpxPltrShadowCandidates', [{ candidate_id: 'CANDIDATE_ANCHOR_V1', configuration_hash: 'sha256:...', status: 'SHADOW_ONLY', immutable: true, no_launch_authorization: true }]));
  add('get', '/v1/4663/reflexive/preflight/ipx-pltr/shadow/status', rh4663('Get shadow observation status', 'Returns public read-only Observation Mode status from the server-native v0.5.1-op worker. It exposes latest READY snapshot, candidate verdict transitions, Identity Market Efficiency when available, and the frozen evidence policy hash. It contains no secrets, performs no writes, and has no transaction capability.', 'IpxPltrShadowObservationStatus', { object_type: 'IPX_PLTR_SHADOW_OBSERVATION_STATUS', latest_ready_snapshot: null, last_refresh_at: null, last_replay_at: null, candidates: { CANDIDATE_NATIVE_V1: { hash: 'sha256:...', verdict: null, previous_verdict: null, transition: null } }, evidence_policy_hash: 'sha256:c7a67c48217864f2ec273e250608eba652b042ec5e6a660efce1e367c2aa9ccd', evidence_window: { satisfied: false }, next_action: 'OBSERVE', transaction_capability: 'NONE_STATUS_ONLY', configurations_touched: false }));
  add('post', '/v1/4663/reflexive/preflight/ipx-pltr/shadow/replay', rh4663('Replay pinned candidates against an explicit PLTR state', 'Reviewer/operator research action. Replays all immutable pinned candidates against exactly one READY_FOR_SIMULATION immutable PLTR snapshot. “latest” is rejected. This creates only SHADOW_PREFLIGHT_RUN research records and cannot execute a transaction.', 'ShadowPreflightRuns', [{ record_type: 'SHADOW_PREFLIGHT_RUN', state_snapshot_id: 'pltr-preflight-52406504-20260902074509000', transaction_capability: 'NONE_SIMULATION_ONLY' }], jsonRequest(objectSchema({ state_snapshot_id: stringSchema() }, ['state_snapshot_id']), { state_snapshot_id: 'pltr-preflight-52406504-20260902074509000' })));
  add('get', '/v1/4663/reflexive/preflight/ipx-pltr/shadow/series/{configuration_hash}', rh4663('Get shadow series and transitions', 'Returns append-only shadow observations, descriptive risk deltas, and non-causal verdict transitions for one pinned configuration hash. It never fabricates missing calendar observations.', 'ShadowSeries', { summary: { evidence_window: { status: 'SHADOW_WINDOW_INSUFFICIENT' }, profitability_statistics: 'NOT_OUTPUT', launch_authorization: false } }, undefined, [pathParam('configuration_hash', 'Immutable pinned candidate configuration hash.')]));
  add('post', '/v1/4663/reflexive/preflight/ipx-pltr/capacity-sweeps', rh4663('Run read-only market capacity sweep', 'Runs broad, versioned market-capacity and range-width research grids against one explicit immutable READY PLTR snapshot. It includes Identity Market Efficiency: modeled usable capacity per percentage point of first-party PLTR concentration. This is not an optimizer, allocation recommendation, venue decision, or execution endpoint.', 'MarketCapacitySweep', { record_type: 'MARKET_CAPACITY_SWEEP', research_only: true, not_an_optimizer: true, no_launch_authorization: true, scenarios: [{ identity_market_efficiency: { label: 'IDENTITY_MARKET_EFFICIENCY', numerator: 'MODELED_USABLE_CAPACITY_USD', denominator: 'FIRST_PARTY_PLTR_CONCENTRATION_PCT', interpretation: 'DESCRIPTIVE_SHADOW_METRIC_NOT_TOKEN_SCORE_OR_RECOMMENDATION' } }] }, jsonRequest(objectSchema({ state_snapshot_id: stringSchema(), candidate_id: stringSchema() }, ['state_snapshot_id']), { state_snapshot_id: 'pltr-preflight-52406504-20260902074509000', candidate_id: 'CANDIDATE_ANCHOR_V1' })));
  add('get', '/v1/4663/reflexive/preflight/ipx-pltr/capacity-sweeps/{id}', rh4663('Get immutable market capacity sweep', 'Returns an immutable MARKET_CAPACITY_SWEEP research artifact with buy/sell impact curves, usable capacity, range effects, supply denominator replay, separately modeled direct-depth assumptions, concentration/capacity frontier points, and Identity Market Efficiency. It is economic architecture research, not Long launch research.', 'MarketCapacitySweep', { record_type: 'MARKET_CAPACITY_SWEEP', research_only: true }, undefined, [pathParam('id', 'Deterministic capacity-sweep identifier.')]));
  add('get', '/v1/4663/reflexive/events', rh4663('List Reflexivity Events', 'Returns explicit versioned market-observation triggers; it contains no trading signals or causal assertion.', 'ReflexivityEventList', { events: [] }));
  add('get', '/v1/4663/reflexive/thesis', rh4663('Get RMM Thesis Board', 'Returns deterministic evidence states: SUPPORTING, MIXED, AGAINST, or INSUFFICIENT_DATA. H6 remains insufficient until a defensible control cohort exists.', 'ReflexiveThesisBoard', { thesis: [{ hypothesis_id: 'H1', state: 'INSUFFICIENT_DATA' }] }));
  add('get', '/v1/4663/reflexive/audits/long-ai-nvda', rh4663('Audit LONG AI/NVDA against Doppler and V4 core state', 'Read-only, fail-closed onchain audit plus v0.4.2 quote topology, aligned provider-context capital/flow shares, immutable inventory history, vault verification state, scoped mission Stock Token footprint, canonical supply events, session/basis context, lifecycle and decomposed thesis evidence. Provider liquidity is never upgraded into launch-position accounting.', 'ReflexiveLongAudit', { canonical_numeraire_verified: true, long_attribution_method: 'LONG_LAUNCHER_AND_AIRLOCK_INTEGRATOR', doppler_generation: 'DOPPLER_HOOK_INITIALIZER_V1', inventory_scope: 'LONG_CANONICAL_LAUNCH_POSITION_INVENTORY', inventory_status: 'AVAILABLE', external_liquidity_coverage: 'PARTIAL', quote_topology: [], stock_quote_volume_share: { status: 'UNAVAILABLE' }, mission_stock_footprint: { scope: 'VERIFIED_AI_NVDA_MISSION_STOCK_FOOTPRINT', status: 'PARTIAL', external_liquidity_included: 'NO' } }));
  add('post', '/internal/4663/reflexive/refresh', rh4663('Refresh Reflexive Radar observations', 'Reviewer-gated at runtime; idempotent RHJ registry sync and evidence refresh. It neither creates protocol receipts nor changes frozen PRINT objects.', 'ReflexiveRadarSnapshot', { assets: [], pairs: [] }));
  add('post', '/internal/4663/reflexive/census/refresh', rh4663('Refresh RMM Category Census', 'Reviewer-gated append-only breadth census. It reads current Radar canonical assets and existing exact pool evidence, and does not invoke deep inventory audits, IPX preflight, transactions, or trading.', 'RmmCategoryCensus', { object_type: 'RMM_CATEGORY_CENSUS', immutable: true }));
  add('post', '/internal/4663/reflexive/watch/claims', rh4663('Create reviewer-gated Watch claim', 'Reviewer-gated append-only intake for REFLEXIVE_WATCH_CLAIM. Duplicate identical claims are rejected; revised source figures create a new claim. This endpoint creates no Radar evidence and no protocol receipt.', 'ReflexiveWatchClaim', { object_type: 'REFLEXIVE_WATCH_CLAIM', immutable: true }, jsonRequest(freeformObject(), { source_type: 'THIRD_PARTY_RESEARCH', source_name: 'Research desk', claim_text: 'A source claim that Radar must audit.' })));
  add('post', '/internal/4663/reflexive/watch/audit-targets', rh4663('Promote Watch claim to Radar audit target', 'Reviewer-gated promotion from Watch claim to RADAR_AUDIT_TARGET. The target answers exact claim, confirm criteria, falsify criteria, onchain objects, time alignment, and missing data. It is a research object, not CALL, RESOLUTION, PRINT, GENESIS, or transaction capability.', 'RadarAuditTarget', { object_type: 'RADAR_AUDIT_TARGET', current_result: 'UNRESOLVED', immutable: true }, jsonRequest(freeformObject(), { claim_id: 'watch-claim-id', case_id: 'AI_NVDA_CAPITAL_VS_FLOW', confirm_criteria: ['same-block totalSupply'], falsify_criteria: ['noncanonical quote contract'] })));
  ...
  const reviewConsole = (summary: string, description: string, payload: string, example: unknown, parameters?: unknown[], requestBody?: JsonSchema) => ({
    ...
    summary,
    description: `Fail-closed internal review surface. Dedicated reviewer bearer auth is required; the feature is disabled by default and never grants providers publication authority. ${description}`,
    security: [{ bearerAuth: [] }],
    ...(parameters ? { parameters } : {}),
    ...(requestBody ? { requestBody } : {}),
    responses: {
  ...
  const classificationPaging = (withStatus = false) => [
    ...
    ...(withStatus ? [{ name: 'status', in: 'query', required: false, schema: enumSchema(['proposed', 'source_required', 'under_review', 'approved', 'rejected', 'superseded', 'archived']) }] : [])
  ];
  const classificationReviewerHeader = { name: 'x-rh-chain-reviewer-id', in: 'header', required: true, description: 'Stable reviewer audit identifier; 1–64 safe identifier characters.', schema: { type: 'string', minLength: 1, maxLength: 64, pattern: '^[A-Za-z0-9._:@-]+$' } };
  add('post', '/internal/4663/pulse/windows/{windowId}/resolve', reviewConsole('Resolve a closed Pulse window', 'Derives the winner from persisted observations and frozen rules. The caller cannot supply a winner. The action is concurrency-protected and idempotent for identical material.', 'Rh4663WindowResolution', { window_id: 'rh4663:2026-08-13', state: 'resolved' }, [pathParam('windowId', 'Closed Pulse window identifier.'), classificationReviewerHeader]));
  add('post', '/internal/4663/prints/{candidateId}/freeze', reviewConsole('Freeze a reviewed 4663 PRINT candidate', 'Regenerates the named candidate, verifies its fingerprint, and writes a one-time immutable market-memory PRINT. Incomplete, stale, or materially disagreed candidates cannot be frozen.', 'Rh4663Print', { print_id: 'rh-print-2026-08-31', status: 'frozen', receipt_kind: 'MARKET_STATE_EVIDENCE' }, [pathParam('candidateId', 'Candidate ID, for example rh-print-candidate-2026-08-31.'), classificationReviewerHeader], jsonRequest(objectSchema({ fingerprint: stringSchema() }, ['fingerprint']), { fingerprint: 'sha256:…' })));
  add('post', '/internal/4663/pulse/windows/{windowId}/publish', reviewConsole('Publish a resolved Pulse window', 'Creates immutable signed RESOLUTION receipts, transitions the result to published once, and safely progresses the window anchor without equating submission with confirmation.', 'Rh4663ResolutionPublication', { resolution: { state: 'published' }, receipts: [], anchor: { state: 'not_submitted' } }, [pathParam('windowId', 'Resolved Pulse window identifier.'), classificationReviewerHeader]));
  add('post', '/internal/4663/intelligence/run', reviewConsole('Run the 4663 intelligence pipeline', 'Runs enabled isolated providers through bounded ingestion, normalization, dedupe, scoring, evidence assembly, candidate policy, and gated publication using the existing durable automation lock.', 'Rh4663IntelligenceRun', { observations: 8, events: 3, candidates: 1, publications: 0 }, [classificationReviewerHeader]));
  add('post', '/internal/4663/intelligence/observations', reviewConsole('Ingest one structured observation', 'Validates and persists a compact evidence-oriented observation. Provider payloads are normalized before they can enter the event layer.', 'Rh4663ProcessingResult', { state: 'candidate', observation_created: true, event_deduplicated: false }, [classificationReviewerHeader], jsonRequest({ $ref: '#/components/schemas/Rh4663RawObservationRequest' }, { provider: 'dexscreener', provider_observation_id: 'snapshot-1:volume', source_type: 'dex_market', observed_at: '2026-08-14T15:42:00.000Z', category: 'MEMES', subject: { subject_type: 'token_contract', subject_id: '0x1111111111111111111111111111111111111111', label: 'CASHCAT' }, metric: 'volume_24h_usd', previous_value: 100000, current_value: 284000, units: 'USD', provider_reference: 'pair-1', source_url: 'https://example.com/pair', confidence: 88, freshness: 'fresh', event_type: 'VOLUME_SPIKE' })));
  add('get', '/internal/4663/intelligence/candidates', reviewConsole('List private Signal candidates', 'Returns protected candidate state, policy reasons, risk classification, outcomes, scores, and heuristic versions. Private candidates are never included in public APIs.', 'Rh4663CandidateList', { candidates: [] }));
  add('post', '/internal/4663/intelligence/candidates/{candidateId}/action', reviewConsole('Review a Signal candidate', 'Uses the existing authenticated reviewer system to publish, hold, reject, or mark a false positive. High-risk candidates cannot bypass evidence or reviewer gates.', 'Rh4663CandidateAction', { candidate: { publication_state: 'held', outcome: 'held' } }, [pathParam('candidateId', 'Private Signal candidate identifier.'), classificationReviewerHeader], jsonRequest(objectSchema({ action: enumSchema(['publish', 'hold', 'reject', 'false_positive']), note: { type: 'string', minLength: 3, maxLength: 1000 } }, ['action', 'note']), { action: 'hold', note: 'Baseline history is still forming.' })));
  add('post', '/internal/4663/intelligence/signals/{signalId}/corrections', reviewConsole('Append a Signal correction', 'Appends CORRECTION, SUPERSEDED, or UPDATED EVIDENCE memory without mutating or deleting the original Signal publication.', 'Rh4663SignalCorrection', { correction_type: 'UPDATED_EVIDENCE', original_publication_hash: 'sha256:…' }, [pathParam('signalId', 'Published Signal Card identifier.'), classificationReviewerHeader], jsonRequest(objectSchema({ correction_type: enumSchema(['CORRECTION', 'SUPERSEDED', 'UPDATED_EVIDENCE']), note: { type: 'string', minLength: 3, maxLength: 1000 }, evidence: arrayOf(freeformObject()) }, ['correction_type', 'note']), { correction_type: 'UPDATED_EVIDENCE', note: 'Provider corrected the source window.' })));
  add('post', '/internal/4663/intelligence/signals/{signalId}/distribution', reviewConsole('Queue future Signal distribution', 'Moves only the separate operational distribution record to queued when the external-distribution gate is explicitly enabled. It does not mutate the Signal publication and this build does not send or post externally.', 'Rh4663DistributionQueue', { signal_id: 'SIGNAL-4663-0084', distribution_state: 'queued', automatic_send: false }, [pathParam('signalId', 'Published Signal Card identifier.'), classificationReviewerHeader]));
  add('post', '/internal/4663/intelligence/backtest', reviewConsole('Backtest 4663 heuristics', 'Replays historical persisted observations deterministically, reports triggers and suppressions with score reasons and heuristic version, and performs zero public writes.', 'Rh4663Backtest', { mode: 'backtest', public_writes: 0, results: [] }, [classificationReviewerHeader], jsonRequest(objectSchema({ since: dateTimeSchema(), until: dateTimeSchema(), heuristic_version: stringSchema() }), {})));
  add('get', '/internal/4663/intelligence/metrics', reviewConsole('Read protected 4663 intelligence metrics', 'Returns aggregate categories, candidate outcomes, publication/hold/review/duplicate rates, provider health, average scores, activation gates, and durable storage state.', 'Rh4663IntelligenceMetrics', { candidates_by_state: {}, provider_health: [], activation: { fail_closed: true } }));
  add('get', '/internal/4663/intelligence/activation', reviewConsole('Inspect Phase 3 activation gates', 'Returns non-secret stage gates, shadow mode, heuristic version, and the explicit Phase 2 production-proof state. It does not imply any external proof step occurred.', 'Rh4663Activation', { code_enabled: true, publication_enabled: false, phase2_production_proof_verified: false, fail_closed: true }));
  add('get', '/internal/rh-chain/classifications', reviewConsole('List authoritative RH Chain classifications', 'Returns bounded exact-contract records from the durable reviewed-classification repository, optionally filtered by review state. This repository is not read by Market Pulse, Market Structure, RCCI, Signal Graph, or public project pages in this phase.', 'RhChainReviewedClassificationListPayload', { items: [], page: 1, page_size: 25, has_more: false, storage: { adapter: 'postgres', durable: true, migrations_run_at_startup: false }, authoritative: true }, classificationPaging(true)));
  add('get', '/internal/rh-chain/classifications/{contract}', reviewConsole('Get an authoritative RH Chain classification', 'Returns the current authoritative record for one exact contract, including reviewer audit metadata.', 'RhChainReviewedClassificationDetailPayload', { classification: { chain: 'robinhood', contract: '0x1111111111111111111111111111111111111111', primary_layer: 'infrastructure', review_status: 'under_review', classification_version: 1 }, storage: { adapter: 'postgres', durable: true, migrations_run_at_startup: false }, authoritative: true }, classificationContract));
  add('post', '/internal/rh-chain/classifications', reviewConsole('Propose an authoritative RH Chain classification', 'Creates a proposal, source-required record, or under-review record only. Provider observations may propose context but cannot approve, promote, or publish it. Re-proposal after a terminal state requires expected_version.', 'RhChainReviewedClassificationDetailPayload', { classification: { chain: 'robinhood', contract: '0x1111111111111111111111111111111111111111', primary_layer: 'infrastructure', review_status: 'proposed', classification_version: 1 }, storage: { adapter: 'postgres', durable: true, migrations_run_at_startup: false }, authoritative: true }, [classificationReviewerHeader], jsonRequest({ $ref: '#/components/schemas/RhChainClassificationProposalRequest' }, { chain: 'robinhood', contract: '0x1111111111111111111111111111111111111111', primary_layer: 'infrastructure', secondary_layers: ['agent'], confidence: 'medium', classification_evidence: [{ evidence_id: 'primary-1', kind: 'primary_source', source_name: 'Project documentation', source_url: 'https://example.com/source', summary: 'Primary documentation supports the classification.', observed_at: '2026-07-19T00:00:00.000Z', content_hash: null }], review_status: 'proposed', source: 'internal_research', manual_override_reason: null, audit_note: 'Created from exact-contract review.' })));
  add('post', '/internal/rh-chain/classifications/{contract}/approve', reviewConsole('Approve an RH Chain classification', 'Transitions proposed, source-required, or under-review memory to approved using classification_version optimistic concurrency. Approval requires retained evidence and authenticated reviewer attribution.', 'RhChainReviewedClassificationDetailPayload', { classification: { contract: '0x1111111111111111111111111111111111111111', review_status: 'approved', classification_version: 2, effective_at: '2026-07-19T00:00:00.000Z' }, authoritative: true }, [...classificationContract, classificationReviewerHeader], jsonRequest({ $ref: '#/components/schemas/RhChainClassificationApprovalRequest' }, { expected_version: 1, audit_note: 'Primary evidence verified.', confidence: 'high' })));
  add('post', '/internal/rh-chain/classifications/{contract}/reject', reviewConsole('Reject an RH Chain classification', 'Rejects a non-terminal review record using classification_version optimistic concurrency and appends an immutable audit event.', 'RhChainReviewedClassificationDetailPayload', { classification: { contract: '0x1111111111111111111111111111111111111111', review_status: 'rejected', classification_version: 2 }, authoritative: true }, [...classificationContract, classificationReviewerHeader], jsonRequest({ $ref: '#/components/schemas/RhChainClassificationDecisionRequest' }, { expected_version: 1, audit_note: 'Evidence did not support the proposed layer.', reason: 'Primary-source mismatch.' })));
  add('post', '/internal/rh-chain/classifications/{contract}/supersede', reviewConsole('Supersede an approved RH Chain classification', 'Moves an approved active record to superseded using classification_version optimistic concurrency. The record immediately leaves the public approved-active view while its complete history remains inspectable.', 'RhChainReviewedClassificationDetailPayload', { classification: { contract: '0x1111111111111111111111111111111111111111', review_status: 'superseded', classification_version: 3, superseded_at: '2026-07-19T00:00:00.000Z' }, authoritative: true }, [...classificationContract, classificationReviewerHeader], jsonRequest({ $ref: '#/components/schemas/RhChainClassificationDecisionRequest' }, { expected_version: 2, audit_note: 'A corrected classification requires review.', reason: 'New primary evidence.' })));
  add('get', '/internal/rh-chain/classifications/{contract}/audit', reviewConsole('Inspect RH Chain classification audit history', 'Returns a bounded newest-first immutable audit history for one exact contract, including the validated record snapshot associated with every transition.', 'RhChainClassificationAuditPayload', { contract: '0x1111111111111111111111111111111111111111', items: [], page: 1, page_size: 25, has_more: false, storage: { adapter: 'postgres', durable: true, migrations_run_at_startup: false }, authoritative: true }, [...classificationContract, ...classificationPaging()]));
  add('get', '/internal/rh-chain/market-structure/cross-layer/conflicts', reviewConsole('Inspect Cross-Layer classification conflicts', 'Read-only reviewer surface for unresolved exact-contract disagreements between existing curated reviewed memory and durable approved classifications. Curated memory remains in force; reviewers resolve the underlying record through the existing protected classification operations.', 'RhChainCrossLayerConflictPayload', { captured_at: '2026-07-19T00:00:00.000Z', conflict_count: 1, conflicts: [{ chain: 'robinhood', contract: '0x1111111111111111111111111111111111111111', conflict_state: 'curated_durable_disagreement', resolution: 'curated_memory_preserved', resolution_route: '/internal/rh-chain/classifications/0x1111111111111111111111111111111111111111' }], source_required_count: 0, source_required: [], warnings: [], bounded_universe: 'Bounded reviewed exact-contract set.', classification_precedence: ['curated_reviewed_memory', 'durable_approved_reviewed_classification', 'provider_context', 'unknown'], resolution_policy: 'Resolve through existing protected reviewed-classification operations.' }));
  add('get', '/internal/rh-chain/attention-quality/assessments', reviewConsole('List stored Attention Quality assessment artifacts', 'Lists bounded receipt-backed Attention Quality artifacts. Assessments derive from persisted market snapshots and reviewed memory; this route never calls a provider.', 'RhChainAttentionQualityReceiptPayload', { assessments: [], receipts: [], provider_requests_in_path: 0 }));
  add('get', '/internal/rh-chain/attention-quality/assessments/{contract}', reviewConsole('Inspect Attention Quality assessment', 'Derives an exact-contract assessment from bounded persisted snapshots. A numeric score is only returned in measurable state.', 'RhChainAttentionQualityReceiptPayload', { assessment: { contract: '0x1111111111111111111111111111111111111111', assessment_state: 'baseline_forming', attention_quality_score: null }, receipts: [], provider_requests_in_path: 0 }, [pathParam('contract', 'Exact token contract address.'), { name: 'window', in: 'query', required: false, schema: { type: 'string', enum: ['24h', '7d', '30d'] } }]));
  add('post', '/internal/rh-chain/attention-quality/assessments/{contract}/receipts', reviewConsole('Create Attention Quality receipt draft', 'Creates an idempotent immutable draft receipt. Data availability never publishes a receipt.', 'RhChainAttentionQualityReceiptPayload', { receipt: { receipt_id: 'rhaq_example', status: 'draft' } }, [pathParam('contract', 'Exact token contract address.'), classificationReviewerHeader], jsonRequest(objectSchema({ window: { type: 'string', enum: ['24h', '7d', '30d'] } }), { window: '7d' })));
  add('post', '/internal/rh-chain/attention-quality/receipts/{receipt_id}/publish', reviewConsole('Publish Attention Quality receipt', 'Publishes a reviewer-approved immutable receipt; it never alters the underlying snapshots or classifications.', 'RhChainAttentionQualityReceiptPayload', { receipt: { receipt_id: 'rhaq_example', status: 'published' } }, [pathParam('receipt_id', 'Attention Quality receipt identifier.'), classificationReviewerHeader]));
  add('post', '/internal/rh-chain/attention-quality/receipts/{receipt_id}/reject', reviewConsole('Reject Attention Quality receipt', 'Marks a draft receipt rejected without deleting its immutable evidence artifact.', 'RhChainAttentionQualityReceiptPayload', { receipt: { receipt_id: 'rhaq_example', status: 'rejected' } }, [pathParam('receipt_id', 'Attention Quality receipt identifier.'), classificationReviewerHeader]));
  ...
  add('get', '/internal/rh-chain/operational-readiness', reviewConsole('Inspect RH Chain production readiness', 'Returns migration signatures, storage/index readiness, feature dependencies, persisted snapshot freshness, bounded reviewed-memory coverage, and explicit blockers. It makes zero provider requests and never exposes connection strings, tokens, reviewer data, or database internals.', 'RhChainOperationalReadinessPayload', { database: { reachable: true, required_tables_present: true, required_indexes_present: true }, migrations: { migration_runner: 'external_only', pending_migrations: [] }, critical_incident_state: 'clear', provider_requests_in_path: 0 }));
  ...
  const projectParams = [pathParam('project_id', 'RH Chain project identifier.'), classificationReviewerHeader];
  const relationshipParams = [...projectParams, pathParam('relationship_id', 'Contract relationship identifier.')];
  const verdictParams = [...projectParams, pathParam('verdict_id', 'Private verdict draft identifier.')];
  const receiptParams = [...projectParams, pathParam('receipt_id', 'Private intelligence receipt identifier.')];
  add('get', '/internal/rh-chain/projects/{project_id}/contracts', reviewConsole('List project contract relationships', 'Lists reviewer-controlled exact-contract identity relationships. Tickers and provider data are never identity authority.', 'RhChainProjectContractRelationshipListPayload', { relationships: [] }, projectParams));
  add('post', '/internal/rh-chain/projects/{project_id}/contracts', reviewConsole('Link a project contract for review', 'Creates a reviewed, non-public contract relationship. expected_version protects the project identity record from stale writes.', 'RhChainProjectContractRelationshipPayload', { relationship_id: 'rhrel_example', verification_state: 'reviewed' }, projectParams, jsonRequest(objectSchema({ expected_version: { type: 'integer', minimum: 1 }, exact_contract: { type: 'string', pattern: '^0x[0-9a-fA-F]{40}$' }, relationship_type: enumSchema(['primary', 'token', 'product', 'protocol', 'treasury', 'liquidity', 'executor', 'deprecated', 'unknown']), evidence_references: { type: 'array', items: stringSchema() } }, ['expected_version', 'exact_contract', 'relationship_type']), { expected_version: 1, exact_contract: '0x1111111111111111111111111111111111111111', relationship_type: 'token', evidence_references: [] })));
  add('post', '/internal/rh-chain/projects/{project_id}/contracts/{relationship_id}/verify', reviewConsole('Verify a contract relationship', 'Verification is independent from claim verification and uses relationship expected_version concurrency.', 'RhChainProjectContractRelationshipPayload', { relationship_id: 'rhrel_example', verification_state: 'verified' }, relationshipParams, jsonRequest(objectSchema({ expected_version: { type: 'integer', minimum: 1 }, evidence_references: { type: 'array', items: stringSchema() }, reason: stringSchema() }, ['expected_version', 'reason']), { expected_version: 1, evidence_references: [], reason: 'Explorer and independent documentation match.' })));
  add('post', '/internal/rh-chain/projects/{project_id}/contracts/{relationship_id}/supersede', reviewConsole('Supersede a contract relationship', 'Preserves the incorrect relationship and links it to a verified replacement; no contract reassignment is silent.', 'RhChainProjectContractRelationshipPayload', { relationship_id: 'rhrel_example', verification_state: 'superseded' }, relationshipParams, jsonRequest(objectSchema({ expected_version: { type: 'integer', minimum: 1 }, replacement_relationship_id: stringSchema(), reason: stringSchema() }, ['expected_version', 'replacement_relationship_id', 'reason']), { expected_version: 2, replacement_relationship_id: 'rhrel_replacement', reason: 'Corrected primary contract.' })));
  add('get', '/internal/rh-chain/projects/{project_id}/contracts/{relationship_id}/audit', reviewConsole('Inspect contract relationship audit', 'Returns immutable reviewer actions for one contract relationship.', 'RhChainProjectAuditPayload', { audit: [] }, relationshipParams));
  add('post', '/internal/rh-chain/projects/{project_id}/verdicts/{verdict_id}/reject', reviewConsole('Reject verdict draft', 'Retains the private draft and attached evidence, but makes it permanently unpublishable.', 'RhChainProjectVerdictPayload', { verdict_id: 'rhverdict_example', verdict_state: 'rejected' }, verdictParams, jsonRequest(objectSchema({ expected_version: { type: 'integer', minimum: 1 }, rejection_reason: stringSchema() }, ['expected_version', 'rejection_reason']), { expected_version: 1, rejection_reason: 'Required evidence is incomplete.' })));
  add('post', '/internal/rh-chain/projects/{project_id}/intelligence-receipts/{receipt_id}/reject', reviewConsole('Reject intelligence receipt draft', 'Retains the private receipt draft, but permanently prevents publication. Published receipts are never editable or rejectable.', 'RhChainIntelligenceReceiptPayload', { receipt_id: 'rhir_example', reviewer_publication_state: 'rejected' }, receiptParams, jsonRequest(objectSchema({ expected_version: { type: 'integer', minimum: 1 }, rejection_reason: stringSchema() }, ['expected_version', 'rejection_reason']), { expected_version: 1, rejection_reason: 'The draft needs stronger source support.' })));
  ...
  return {
    ...
    tags: [
      ...
      , { name: 'Infopunks //4663', description: 'Canonical mobile-first RH Chain operating surface: Pulse, daily intelligence, signals, protocol receipts, and Genesis provenance.' }
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  const strictObject = (properties: Record<string, unknown>, required: string[] = []) => ({ ...objectSchema(properties, required), additionalProperties: false });
  const rhChainClassificationState = enumSchema(['proposed', 'source_required', 'under_review', 'approved', 'rejected', 'superseded', 'archived']);
  const rhChainClassificationLayer = enumSchema(['meme', 'rwa', 'agent', 'infrastructure', 'defi', 'consumer', 'unknown']);
  const rhChainClassificationSecondaryLayer = enumSchema(['meme', 'rwa', 'agent', 'infrastructure', 'defi', 'lending', 'payments', 'data', 'identity', 'launchpad', 'energy', 'trading', 'consumer', 'ai-narrative', 'unknown']);
  const rhChainClassificationSource = enumSchema(['manual_review', 'internal_research', 'provider_observation', 'community_submission', 'review_pipeline']);
  const rhChainExactContract = { type: 'string', pattern: '^0x[0-9a-fA-F]{40}$' };
  const rhChainClassificationEvidence = strictObject({ evidence_id: { type: 'string', minLength: 1, maxLength: 128 }, kind: enumSchema(['primary_source', 'onchain', 'market_context', 'social', 'receipt', 'manual_note']), source_name: { type: 'string', minLength: 1, maxLength: 160 }, source_url: { type: ['string', 'null'], format: 'uri', maxLength: 1000 }, summary: { type: 'string', minLength: 1, maxLength: 2000 }, observed_at: dateTimeSchema(), content_hash: { type: ['string', 'null'], minLength: 8, maxLength: 160 } }, ['evidence_id', 'kind', 'source_name', 'source_url', 'summary', 'observed_at', 'content_hash']);
  const rhChainReviewerAudit = strictObject({ last_reviewer_id: { type: 'string', minLength: 1, maxLength: 64 }, last_action: enumSchema(['proposed', 'approved', 'rejected', 'superseded']), last_action_at: dateTimeSchema(), audit_note: { type: 'string', minLength: 1, maxLength: 2000 } }, ['last_reviewer_id', 'last_action', 'last_action_at', 'audit_note']);
  const rhChainClassificationFields = { chain: { const: 'robinhood' }, contract: rhChainExactContract, primary_layer: rhChainClassificationLayer, secondary_layers: { type: 'array', maxItems: 5, uniqueItems: true, items: rhChainClassificationSecondaryLayer }, confidence: enumSchema(['low', 'medium', 'high']), classification_evidence: { type: 'array', maxItems: 50, items: { $ref: '#/components/schemas/RhChainClassificationEvidence' } }, classification_version: { type: 'integer', minimum: 1 }, review_status: rhChainClassificationState, effective_at: { type: ['string', 'null'], format: 'date-time' }, superseded_at: { type: ['string', 'null'], format: 'date-time' }, source: rhChainClassificationSource, created_at: dateTimeSchema(), updated_at: dateTimeSchema() };
  const rhChainClassificationRequired = ['chain', 'contract', 'primary_layer', 'secondary_layers', 'confidence', 'classification_evidence', 'classification_version', 'review_status', 'effective_at', 'superseded_at', 'source', 'created_at', 'updated_at'];
  const rhChainClassificationStorage = strictObject({ adapter: enumSchema(['memory', 'postgres']), durable: booleanSchema(), migrations_run_at_startup: { const: false } }, ['adapter', 'durable', 'migrations_run_at_startup']);
  ...
  return {
    ...
    Rh4663UtcDayObservation: objectSchema({ observation_id: stringSchema(), chain_id: { const: 4663 }, date: { type: 'string', format: 'date' }, metric: enumSchema(['transactions_utc_day', 'dex_volume_utc_day_usd']), value: { type: 'number', minimum: 0 }, unit: enumSchema(['COUNT', 'USD']), provider: stringSchema(), source_url: { type: 'string', format: 'uri' }, observed_at: dateTimeSchema(), fetched_at: dateTimeSchema(), window_start: dateTimeSchema(), window_end: dateTimeSchema(), window_type: { const: 'UTC_CALENDAR_DAY' }, methodology: stringSchema(), freshness: { const: 'HISTORICAL_FINAL' }, status: { const: 'FINAL' }, revision: { type: 'integer', minimum: 1 }, revision_of: nullableString(), source_metadata: freeformObject(), created_at: dateTimeSchema(), finalized_at: { type: ['string', 'null'], format: 'date-time' } }, ['observation_id', 'chain_id', 'date', 'metric', 'value', 'unit', 'provider', 'source_url', 'observed_at', 'fetched_at', 'window_start', 'window_end', 'window_type', 'methodology', 'freshness', 'status', 'revision', 'revision_of', 'source_metadata', 'created_at', 'finalized_at']),
    Rh4663UtcDayObservations: objectSchema({ date: { type: 'string', format: 'date' }, transactions: { anyOf: [{ $ref: '#/components/schemas/Rh4663UtcDayObservation' }, { type: 'null' }] }, dex_volume: { anyOf: [{ $ref: '#/components/schemas/Rh4663UtcDayObservation' }, { type: 'null' }] }, status: enumSchema(['FINALIZED', 'INCOMPLETE']), warnings: arrayOf(stringSchema()), storage: freeformObject() }, ['date', 'transactions', 'dex_volume', 'status', 'warnings', 'storage']),
    Rh4663UtcDayRefresh: objectSchema({ date: { type: 'string', format: 'date' }, transactions: { anyOf: [{ $ref: '#/components/schemas/Rh4663UtcDayObservation' }, { type: 'null' }] }, dex_volume: { anyOf: [{ $ref: '#/components/schemas/Rh4663UtcDayObservation' }, { type: 'null' }] }, status: enumSchema(['FINALIZED', 'INCOMPLETE']), warnings: arrayOf(stringSchema()), requested_by: stringSchema(), storage: freeformObject() }, ['date', 'transactions', 'dex_volume', 'status', 'warnings', 'requested_by', 'storage']),
    Rh4663PulsePayloadRequest: strictObject({ wallet: rhChainExactContract, rotation: enumSchema(['MEMES', 'STOCK_TOKENS', 'RWA_DEFI', 'STABLES', 'NO_QUALIFIED_ROTATION']), confidence: { type: 'integer', minimum: 1, maximum: 100 }, evidence_digest: { type: ['string', 'null'], pattern: '^0x[0-9a-fA-F]{64}$' }, window_id: stringSchema() }, ['wallet', 'rotation', 'confidence']),
    Rh4663PulseCallRequest: strictObject({ wallet: rhChainExactContract, rotation: enumSchema(['MEMES', 'STOCK_TOKENS', 'RWA_DEFI', 'STABLES', 'NO_QUALIFIED_ROTATION']), confidence: { type: 'integer', minimum: 1, maximum: 100 }, evidence_digest: { type: ['string', 'null'], pattern: '^0x[0-9a-fA-F]{64}$' }, window_id: stringSchema(), signature: { type: 'string', pattern: '^0x[0-9a-fA-F]+$' } }, ['wallet', 'rotation', 'confidence', 'signature']),
    Rh4663CanonicalPayload: objectSchema({ payload: freeformObject(), canonical_serialization: stringSchema(), payload_hash: stringSchema() }, ['payload', 'canonical_serialization', 'payload_hash']),
    Rh4663CallReceipt: objectSchema({ receipt_id: stringSchema(), receipt_kind: { const: 'PROTOCOL_RECEIPT' }, protocol_receipt_type: { const: 'CALL' }, immutable: { const: true }, created_at: dateTimeSchema(), wallet: rhChainExactContract, window_id: stringSchema(), rotation: enumSchema(['MEMES', 'STOCK_TOKENS', 'RWA_DEFI', 'STABLES', 'NO_QUALIFIED_ROTATION']), confidence: { type: 'integer', minimum: 1, maximum: 100 }, evidence_digest: nullableString(), canonical_payload: freeformObject(), canonical_serialization: stringSchema(), payload_hash: stringSchema(), signature: stringSchema(), signature_verified: { const: true }, genesis_eligible: booleanSchema(), genesis_ordinal: { type: ['integer', 'null'], minimum: 1, maximum: 4663 }, resolution_compatibility: { const: 'deterministic_v1' } }, ['receipt_id', 'receipt_kind', 'protocol_receipt_type', 'immutable', 'created_at', 'wallet', 'window_id', 'rotation', 'confidence', 'canonical_payload', 'canonical_serialization', 'payload_hash', 'signature', 'signature_verified', 'genesis_eligible', 'resolution_compatibility']),
    Rh4663ResolutionReceipt: objectSchema({ receipt_id: { type: 'string', pattern: '^IP-RES-[0-9A-F]+$' }, receipt_kind: { const: 'PROTOCOL_RECEIPT' }, protocol_receipt_type: { const: 'RESOLUTION' }, immutable: { const: true }, created_at: dateTimeSchema(), wallet: rhChainExactContract, window_id: stringSchema(), call_receipt_id: stringSchema(), call_receipt_hash: stringSchema(), called_category: stringSchema(), resolved_category: stringSchema(), outcome: enumSchema(['CORRECT', 'INCORRECT']), confidence: { type: 'integer', minimum: 1, maximum: 100 }, observation_window: freeformObject(), dependencies: freeformObject(), acceptance_root: stringSchema(), canonical_payload: freeformObject(), canonical_serialization: stringSchema(), payload_hash: stringSchema(), signature: stringSchema(), signature_verified: { const: true }, resolution_signer: rhChainExactContract, signer_key_id: stringSchema(), publication_state: { const: 'published' }, resolved_at: dateTimeSchema() }, ['receipt_id', 'receipt_kind', 'protocol_receipt_type', 'immutable', 'created_at', 'wallet', 'window_id', 'call_receipt_id', 'call_receipt_hash', 'called_category', 'resolved_category', 'outcome', 'confidence', 'observation_window', 'dependencies', 'acceptance_root', 'canonical_payload', 'canonical_serialization', 'payload_hash', 'signature', 'signature_verified', 'resolution_signer', 'signer_key_id', 'publication_state', 'resolved_at']),
    Rh4663ProtocolReceipt: { oneOf: [{ $ref: '#/components/schemas/Rh4663CallReceipt' }, { $ref: '#/components/schemas/Rh4663ResolutionReceipt' }] },
    Rh4663Pulse: objectSchema({ window: objectSchema({ window_id: stringSchema(), opens_at: dateTimeSchema(), closes_at: dateTimeSchema(), closes_at_exclusive: { const: true }, semantics: { const: 'fixed_utc_day' } }), consensus: freeformObject(), options: arrayOf(stringSchema()), mechanics: freeformObject() }, ['window', 'consensus', 'options', 'mechanics']),
    Rh4663Window: objectSchema({ window: freeformObject(), state: enumSchema(['open', 'closed', 'resolved', 'published']), consensus: freeformObject(), acceptance: { anyOf: [freeformObject(), { type: 'null' }] }, anchor: { anyOf: [freeformObject(), { type: 'null' }] }, resolution: { anyOf: [{ $ref: '#/components/schemas/Rh4663WindowResolution' }, { type: 'null' }] }, storage: freeformObject() }, ['window', 'state', 'consensus', 'acceptance', 'anchor', 'resolution', 'storage']),
    Rh4663WindowResolution: objectSchema({ version: { const: 'infopunks.rh-pulse.resolution.v1' }, window_id: stringSchema(), state: enumSchema(['resolved', 'published']), pulse_window: freeformObject(), observation_window: freeformObject(), resolved_category: enumSchema(['MEMES', 'STOCK_TOKENS', 'RWA_DEFI', 'STABLES', 'NO_QUALIFIED_ROTATION']), category_scores: freeformObject(), determination: freeformObject(), observations: arrayOf(freeformObject()), observation_set_hash: stringSchema(), provider_state: freeformObject(), dependencies: freeformObject(), acceptance: freeformObject(), anchor: freeformObject(), consensus: freeformObject(), resolved_at: dateTimeSchema(), published_at: { type: ['string', 'null'], format: 'date-time' }, immutable_after_publication: { const: true } }, ['version', 'window_id', 'state', 'pulse_window', 'observation_window', 'resolved_category', 'category_scores', 'determination', 'observations', 'observation_set_hash', 'provider_state', 'dependencies', 'acceptance', 'anchor', 'consensus', 'resolved_at', 'immutable_after_publication']),
    Rh4663MerkleProof: objectSchema({ version: { const: 'infopunks.rh-pulse.acceptance-merkle.v1' }, receipt_id: stringSchema(), call_receipt_hash: stringSchema(), leaf_hash: stringSchema(), leaf_index: { type: 'integer', minimum: 0 }, receipt_count: { type: 'integer', minimum: 1 }, acceptance_root: stringSchema(), proof: arrayOf(freeformObject()), verified: booleanSchema(), anchor: freeformObject() }, ['version', 'receipt_id', 'call_receipt_hash', 'leaf_hash', 'leaf_index', 'receipt_count', 'acceptance_root', 'proof', 'verified', 'anchor']),
    Rh4663Reputation: objectSchema({ wallet: rhChainExactContract, calls: { type: 'integer', minimum: 0 }, resolved_calls: { type: 'integer', minimum: 0 }, correct_calls: { type: 'integer', minimum: 0 }, accuracy: { type: ['number', 'null'], minimum: 0, maximum: 1 }, current_streak: { type: 'integer', minimum: 0 }, genesis_position: { type: ['integer', 'null'], minimum: 1, maximum: 4663 }, first_call_date: { type: ['string', 'null'], format: 'date' }, last_call_date: { type: ['string', 'null'], format: 'date' }, definitions: freeformObject(), evidence: arrayOf(freeformObject()) }, ['wallet', 'calls', 'resolved_calls', 'correct_calls', 'accuracy', 'current_streak', 'genesis_position', 'definitions', 'evidence']),
    Rh4663ProofReceiptLink: objectSchema({ receipt_id: stringSchema(), receipt_url: stringSchema(), resolution_receipt_id: nullableString(), resolution_receipt_url: nullableString() }, ['receipt_id', 'receipt_url', 'resolution_receipt_id', 'resolution_receipt_url']),
    Rh4663ProofCategory: objectSchema({ calls: { type: 'integer', minimum: 0 }, resolved: { type: 'integer', minimum: 0 }, correct: { type: 'integer', minimum: 0 }, incorrect: { type: 'integer', minimum: 0 }, unresolved: { type: 'integer', minimum: 0 }, accuracy: { type: ['number', 'null'], minimum: 0, maximum: 1 }, sample_status: enumSchema(['MEANINGFUL', 'INSUFFICIENT_SAMPLE']), receipt_links: arrayOf({ $ref: '#/components/schemas/Rh4663ProofReceiptLink' }) }, ['calls', 'resolved', 'correct', 'incorrect', 'unresolved', 'accuracy', 'sample_status', 'receipt_links']),
    Rh4663ProofProfile: objectSchema({ object_type: { const: 'PROOF_PROFILE' }, profile_version: stringSchema(), wallet: rhChainExactContract, display_name: stringSchema(), calls: { type: 'integer', minimum: 0 }, resolved: { type: 'integer', minimum: 0 }, correct: { type: 'integer', minimum: 0 }, incorrect: { type: 'integer', minimum: 0 }, unresolved: { type: 'integer', minimum: 0 }, accuracy: { type: ['number', 'null'], minimum: 0, maximum: 1 }, high_confidence_accuracy: { type: ['number', 'null'], minimum: 0, maximum: 1 }, high_confidence: freeformObject(), category_breakdown: freeformObject(), best_supported_category: freeformObject(), genesis: { anyOf: [freeformObject(), { type: 'null' }] }, genesis_receipt: { anyOf: [freeformObject(), { type: 'null' }] }, recent_calls: arrayOf(freeformObject()), streak: freeformObject(), methodology: freeformObject(), receipt_links: arrayOf({ $ref: '#/components/schemas/Rh4663ProofReceiptLink' }) }, ['object_type', 'profile_version', 'wallet', 'display_name', 'calls', 'resolved', 'correct', 'incorrect', 'unresolved', 'accuracy', 'high_confidence_accuracy', 'high_confidence', 'category_breakdown', 'best_supported_category', 'genesis', 'genesis_receipt', 'recent_calls', 'streak', 'methodology', 'receipt_links']),
    Rh4663MyProof: objectSchema({ authenticated: booleanSchema(), profile: { anyOf: [{ $ref: '#/components/schemas/Rh4663ProofProfile' }, { type: 'null' }] }, my_4663_version: stringSchema() }, ['authenticated', 'profile', 'my_4663_version']),
    Rh4663ShareObject: objectSchema({ object_version: stringSchema(), object_type: enumSchema(['pending_call', 'resolved_correct_call', 'resolved_incorrect_call']), call: freeformObject(), resolution: { anyOf: [freeformObject(), { type: 'null' }] }, genesis_position: { type: ['integer', 'null'] }, record: freeformObject(), proof_url: stringSchema(), call_receipt_url: stringSchema(), resolution_receipt_url: nullableString(), inclusion_proof_url: stringSchema(), images: freeformObject(), verified_property: stringSchema() }, ['object_version', 'object_type', 'call', 'resolution', 'record', 'proof_url', 'call_receipt_url', 'inclusion_proof_url', 'images', 'verified_property']),
    Rh4663WindowShareObject: objectSchema({ object_version: stringSchema(), object_type: enumSchema(['window_consensus', 'window_result']), window_id: stringSchema(), total_calls: { type: 'integer', minimum: 0 }, consensus_category: nullableString(), consensus_percentage: { type: 'number', minimum: 0, maximum: 100 }, resolved_category: nullableString(), consensus_correct: { type: ['boolean', 'null'] }, primary_category: nullableString(), proof_url: stringSchema(), images: freeformObject(), verified_property: stringSchema() }, ['object_version', 'object_type', 'window_id', 'total_calls', 'consensus_category', 'consensus_percentage', 'resolved_category', 'consensus_correct', 'primary_category', 'proof_url', 'images', 'verified_property']),
    Rh4663ResolutionPublication: objectSchema({ resolution: { $ref: '#/components/schemas/Rh4663WindowResolution' }, receipts: arrayOf({ $ref: '#/components/schemas/Rh4663ResolutionReceipt' }), anchor: freeformObject(), requested_by: stringSchema() }, ['resolution', 'receipts', 'anchor']),
    Rh4663SignalSubmissionRequest: strictObject({ title: { type: 'string', minLength: 3, maxLength: 180 }, category: enumSchema(['meme', 'nft_culture', 'utility', 'agent', 'stock_token', 'defi', 'wallet', 'liquidity', 'risk', 'integration', 'other']), thesis: { type: 'string', minLength: 8, maxLength: 2000 }, submitter: { type: 'string', minLength: 1, maxLength: 120 }, source_url: { type: 'string', format: 'uri' }, evidence_note: { type: 'string', minLength: 3, maxLength: 1000 } }, ['title', 'category', 'thesis', 'submitter', 'source_url']),
    Rh4663SignalEvidenceRequest: strictObject({ url: { type: 'string', format: 'uri' }, label: { type: 'string', minLength: 1, maxLength: 180 }, observed_at: dateTimeSchema() }, ['url', 'label']),
    Rh4663Signal: objectSchema({ signal_id: stringSchema(), representation_kind: { const: 'SIGNAL_CARD' }, title: stringSchema(), category: stringSchema(), thesis: stringSchema(), lifecycle_state: enumSchema(['submitted', 'watching', 'evidence_added', 'confirmed', 'rejected', 'unresolved']), original_submitter: stringSchema(), submitted_at: dateTimeSchema(), updated_at: dateTimeSchema(), evidence: arrayOf(freeformObject()), attribution_immutable: { const: true }, guarantee_notice: stringSchema(), lifecycle_history: arrayOf(freeformObject()) }, ['signal_id', 'representation_kind', 'title', 'category', 'thesis', 'lifecycle_state', 'original_submitter', 'submitted_at', 'updated_at', 'evidence', 'attribution_immutable', 'guarantee_notice', 'lifecycle_history']),
    Rh4663TodayEdition: objectSchema({ edition_id: stringSchema(), date: { type: 'string', format: 'date' }, generated_at: dateTimeSchema(), top_events: arrayOf(freeformObject()), category_flows: arrayOf(freeformObject()), key_signal: stringSchema(), rh_pulse_consensus: { anyOf: [freeformObject(), { type: 'null' }] }, rh_pulse: { anyOf: [freeformObject(), { type: 'null' }] }, evidence_references: arrayOf(freeformObject()), confidence: { type: 'integer', minimum: 0, maximum: 100 }, source_timestamps: arrayOf(dateTimeSchema()), provider_state: enumSchema(['available', 'stale', 'degraded', 'unavailable']), storage_status: enumSchema(['durable', 'memory', 'unavailable']), archive_path: stringSchema(), data_notice: stringSchema(), edition_state: enumSchema(['draft', 'ready', 'published', 'degraded']), intelligence_signal_ids: arrayOf(stringSchema()) }, ['edition_id', 'date', 'generated_at', 'top_events', 'category_flows', 'key_signal', 'evidence_references', 'confidence', 'source_timestamps', 'provider_state', 'storage_status', 'archive_path', 'data_notice']),
    Rh4663Overview: freeformObject(),
    Rh4663FrontdoorState: objectSchema({ object_type: { const: 'RH_4663_FRONTDOOR_STATE' }, generated_at: dateTimeSchema(), freshness: objectSchema({ state: enumSchema(['VERIFIED', 'MIXED', 'WATCH', 'UNRESOLVED', 'BLOCK', 'DEGRADE', 'INSUFFICIENT_DATA']), source_observed_at: nullableString() }, ['state', 'source_observed_at']), frontdoor_version: objectSchema({ object_type: { const: 'FRONTDOOR_VERSION' }, version: { type: 'integer', minimum: 1 }, changed: arrayOf(stringSchema()), generated_at: dateTimeSchema() }, ['object_type', 'version', 'changed', 'generated_at']), frontdoor_version_durability: enumSchema(['PERSISTENT', 'EPHEMERAL']), now_cards: arrayOf(freeformObject()), watch_cards: arrayOf(freeformObject()), open_loops: arrayOf(freeformObject()), change_events: arrayOf({ $ref: '#/components/schemas/FrontdoorChangeEvent' }), current_call: freeformObject(), proof_summary: freeformObject(), system_status: freeformObject(), source_refs: arrayOf(freeformObject()) }, ['object_type', 'generated_at', 'freshness', 'frontdoor_version', 'frontdoor_version_durability', 'now_cards', 'watch_cards', 'open_loops', 'change_events', 'current_call', 'proof_summary', 'system_status', 'source_refs']),
    My4663State: objectSchema({ object_type: { const: 'MY_4663_STATE' }, follows: arrayOf(freeformObject()), changed_followed_subjects: arrayOf(freeformObject()), followed_open_loops: arrayOf(freeformObject()), followed_now_items: arrayOf(freeformObject()), personal_call_state: { type: ['object', 'null'] }, personal_proof: { type: ['object', 'null'] }, last_seen_my4663_event_id: nullableString(), my_4663_version: stringSchema() }, ['object_type', 'follows', 'changed_followed_subjects', 'followed_open_loops', 'followed_now_items', 'personal_call_state', 'personal_proof', 'last_seen_my4663_event_id', 'my_4663_version']),
    FrontdoorChangeEvent: objectSchema({ object_type: { const: 'FRONTDOOR_CHANGE_EVENT' }, event_id: stringSchema(), frontdoor_version: { type: 'integer', minimum: 1 }, occurred_at: dateTimeSchema(), source_type: stringSchema(), source_ref: freeformObject(), change_type: enumSchema(['NEW', 'UPDATED', 'STATE_TRANSITION', 'CHECKPOINT_REACHED', 'RESOLVED', 'FALSIFIED', 'EVIDENCE_ADDED', 'COVERAGE_CHANGED', 'VERDICT_CHANGED']), headline: stringSchema(), before: { anyOf: [freeformObject(), { type: 'null' }] }, after: freeformObject(), importance: { type: 'integer', minimum: 0 }, deep_link: stringSchema(), source_observed_at: nullableString() }, ['object_type', 'event_id', 'frontdoor_version', 'occurred_at', 'source_type', 'source_ref', 'change_type', 'headline', 'before', 'after', 'importance', 'deep_link', 'source_observed_at']),
    Rh4663MyCall: objectSchema({ authenticated: booleanSchema(), window_id: stringSchema(), window: freeformObject(), has_called: booleanSchema(), call_receipt_reference: nullableString(), selection: nullableString(), confidence: { type: ['integer', 'null'], minimum: 1, maximum: 100 }, submitted_at: { type: ['string', 'null'], format: 'date-time' }, resolution_state: nullableString(), resolution_receipt: { anyOf: [freeformObject(), { type: 'null' }] }, my_4663_version: stringSchema() }, ['authenticated', 'window_id', 'has_called', 'call_receipt_reference', 'selection', 'confidence', 'submitted_at', 'resolution_state', 'resolution_receipt', 'my_4663_version']),
    Rh4663MyChanges: objectSchema({ authenticated: booleanSchema(), resolved_call: { anyOf: [freeformObject(), { type: 'null' }] }, personal_events: arrayOf(freeformObject()), pending_call: { anyOf: [freeformObject(), { type: 'null' }] }, my_4663_version: stringSchema() }, ['authenticated', 'resolved_call', 'personal_events', 'pending_call', 'my_4663_version']),
    Rh4663TodayArchive: objectSchema({ editions: arrayOf({ $ref: '#/components/schemas/Rh4663TodayEdition' }), storage: freeformObject() }, ['editions', 'storage']),
    Rh4663SignalList: objectSchema({ signals: arrayOf({ $ref: '#/components/schemas/Published4663Signal' }), watching: arrayOf({ $ref: '#/components/schemas/Rh4663Signal' }), signal_hunt: arrayOf({ $ref: '#/components/schemas/Rh4663Signal' }), archive_count: { type: 'integer', minimum: 0 }, publication_state: { const: 'published' }, provider_requests_in_path: { const: 0 }, guarantee_notice: stringSchema() }, ['signals', 'watching', 'signal_hunt', 'archive_count', 'publication_state', 'provider_requests_in_path', 'guarantee_notice']),
    Published4663Signal: objectSchema({ signal_id: stringSchema(), candidate_id: stringSchema(), representation_kind: { const: 'SIGNAL_CARD' }, immutable: { const: true }, event_ids: arrayOf(stringSchema()), category: enumSchema(['MEMES', 'STOCK_TOKENS', 'RWA_DEFI', 'STABLES', 'CULTURE_NFT', 'UTILITY', 'AGENT', 'WALLET', 'LIQUIDITY', 'INTEGRATION', 'SECURITY', 'OTHER']), signal_type: stringSchema(), subjects: arrayOf(freeformObject()), headline: stringSchema(), summary: stringSchema(), significance_score: { type: 'integer', minimum: 0, maximum: 100 }, significance_components: freeformObject(), anomaly_score: { type: 'integer', minimum: 0, maximum: 100 }, anomaly_basis: stringSchema(), evidence: arrayOf(freeformObject()), source_count: { type: 'integer', minimum: 1 }, heuristic_version: stringSchema(), detected_at: dateTimeSchema(), published_at: dateTimeSchema(), publication_hash: { type: 'string', pattern: '^sha256:[0-9a-f]{64}$' }, proof_url: stringSchema(), share: freeformObject(), distribution_state: enumSchema(['not_queued', 'queued', 'sent', 'failed']), finder_attribution: { anyOf: [freeformObject(), { type: 'null' }] }, semantics: stringSchema(), correction_state: stringSchema(), corrections: arrayOf(freeformObject()) }, ['signal_id', 'candidate_id', 'representation_kind', 'immutable', 'event_ids', 'category', 'signal_type', 'subjects', 'headline', 'summary', 'significance_score', 'significance_components', 'anomaly_score', 'anomaly_basis', 'evidence', 'source_count', 'heuristic_version', 'detected_at', 'published_at', 'publication_hash', 'proof_url', 'share', 'semantics']),
    Rh4663SignalEvidenceBundle: objectSchema({ signal_id: stringSchema(), publication_hash: stringSchema(), evidence: arrayOf(freeformObject()), corrections: arrayOf(freeformObject()), traceability: stringSchema() }, ['signal_id', 'publication_hash', 'evidence', 'corrections', 'traceability']),
    Rh4663MarketLens: objectSchema({ lens: stringSchema(), source: { const: 'shared_normalized_4663_event_system' }, signals: arrayOf({ $ref: '#/components/schemas/Published4663Signal' }), signal_count: { type: 'integer', minimum: 0 }, provider_requests_in_path: { const: 0 } }, ['lens', 'source', 'signals', 'signal_count', 'provider_requests_in_path']),
    Rh4663RotationLens: objectSchema({ object_type: { const: 'LIVE_ROTATION_SIGNAL' }, protocol_resolution: { const: false }, semantics: stringSchema(), heuristic_version: stringSchema(), flows: arrayOf(freeformObject()), leader: nullableString() }, ['object_type', 'protocol_resolution', 'semantics', 'heuristic_version', 'flows', 'leader']),
    Rh4663RawObservationRequest: strictObject({ provider: stringSchema(), provider_observation_id: stringSchema(), source_type: enumSchema(['dex_market', 'chain_explorer', 'defi_metrics', 'price_index', 'internal_snapshot', 'community', 'reviewed_evidence']), observed_at: dateTimeSchema(), category: enumSchema(['MEMES', 'STOCK_TOKENS', 'RWA_DEFI', 'STABLES', 'CULTURE_NFT', 'UTILITY', 'AGENT', 'WALLET', 'LIQUIDITY', 'INTEGRATION', 'SECURITY', 'OTHER']), subject: freeformObject(), metric: stringSchema(), current_value: {}, previous_value: {}, units: nullableString(), provider_reference: stringSchema(), source_url: stringSchema(), confidence: { type: 'integer', minimum: 0, maximum: 100 }, freshness: enumSchema(['fresh', 'stale', 'expired']), event_type: stringSchema(), baseline: freeformObject(), dimensions: freeformObject() }, ['provider', 'provider_observation_id', 'source_type', 'observed_at', 'category', 'subject', 'metric', 'current_value', 'provider_reference', 'source_url', 'confidence']),
    Rh4663ProcessingResult: freeformObject(),
    Rh4663IntelligenceRun: freeformObject(),
    Rh4663CandidateList: freeformObject(),
    Rh4663CandidateAction: freeformObject(),
    Rh4663SignalCorrection: freeformObject(),
    Rh4663DistributionQueue: freeformObject(),
    Rh4663Backtest: freeformObject(),
    Rh4663IntelligenceMetrics: freeformObject(),
    Rh4663Activation: freeformObject(),
    Rh4663EventList: objectSchema({ events: arrayOf(freeformObject()) }, ['events']),
    Rh4663ReceiptList: objectSchema({ receipt_kind: { const: 'PROTOCOL_RECEIPT' }, receipts: arrayOf({ $ref: '#/components/schemas/Rh4663CallReceipt' }), immutable: { const: true } }, ['receipt_kind', 'receipts', 'immutable']),
    ...
    RhChainClassificationEvidence: rhChainClassificationEvidence,
    RhChainClassificationReviewerAudit: rhChainReviewerAudit,
    RhChainReviewedClassification: strictObject({ ...rhChainClassificationFields, reviewer_audit: { $ref: '#/components/schemas/RhChainClassificationReviewerAudit' }, manual_override_reason: { type: ['string', 'null'], minLength: 1, maxLength: 2000 } }, [...rhChainClassificationRequired, 'reviewer_audit', 'manual_override_reason']),
    RhChainPublicClassification: strictObject(rhChainClassificationFields, rhChainClassificationRequired),
    RhChainClassificationAuditEvent: strictObject({ event_id: { type: 'string', format: 'uuid' }, chain: { const: 'robinhood' }, contract: rhChainExactContract, classification_version: { type: 'integer', minimum: 1 }, action: enumSchema(['proposed', 'approved', 'rejected', 'superseded']), from_status: { anyOf: [rhChainClassificationState, { type: 'null' }] }, to_status: rhChainClassificationState, reviewer_id: { type: 'string', minLength: 1, maxLength: 64 }, reason: { type: 'string', minLength: 1, maxLength: 2000 }, occurred_at: dateTimeSchema(), record: { $ref: '#/components/schemas/RhChainReviewedClassification' } }, ['event_id', 'chain', 'contract', 'classification_version', 'action', 'from_status', 'to_status', 'reviewer_id', 'reason', 'occurred_at', 'record']),
    RhChainClassificationProposalRequest: strictObject({ chain: { const: 'robinhood', default: 'robinhood' }, contract: rhChainExactContract, primary_layer: rhChainClassificationLayer, secondary_layers: { type: 'array', maxItems: 5, uniqueItems: true, items: rhChainClassificationSecondaryLayer, default: [] }, confidence: enumSchema(['low', 'medium', 'high']), classification_evidence: { type: 'array', maxItems: 50, items: { $ref: '#/components/schemas/RhChainClassificationEvidence' }, default: [] }, review_status: enumSchema(['proposed', 'source_required', 'under_review']), source: rhChainClassificationSource, manual_override_reason: { type: ['string', 'null'], minLength: 1, maxLength: 2000, default: null }, audit_note: { type: 'string', minLength: 1, maxLength: 2000 }, expected_version: { type: 'integer', minimum: 1 } }, ['contract', 'primary_layer', 'confidence', 'source', 'audit_note']),
    RhChainClassificationApprovalRequest: strictObject({ expected_version: { type: 'integer', minimum: 1 }, audit_note: { type: 'string', minLength: 1, maxLength: 2000 }, confidence: enumSchema(['low', 'medium', 'high']), classification_evidence: { type: 'array', maxItems: 50, items: { $ref: '#/components/schemas/RhChainClassificationEvidence' } }, manual_override_reason: { type: ['string', 'null'], minLength: 1, maxLength: 2000 } }, ['expected_version', 'audit_note']),
    RhChainClassificationDecisionRequest: strictObject({ expected_version: { type: 'integer', minimum: 1 }, audit_note: { type: 'string', minLength: 1, maxLength: 2000 }, reason: { type: 'string', minLength: 1, maxLength: 2000 } }, ['expected_version', 'audit_note', 'reason']),
    RhChainReviewedClassificationListPayload: strictObject({ items: arrayOf({ $ref: '#/components/schemas/RhChainReviewedClassification' }), page: { type: 'integer', minimum: 1 }, page_size: { type: 'integer', minimum: 1, maximum: 100 }, has_more: booleanSchema(), storage: rhChainClassificationStorage, authoritative: { const: true } }, ['items', 'page', 'page_size', 'has_more', 'storage', 'authoritative']),
    RhChainReviewedClassificationDetailPayload: strictObject({ classification: { $ref: '#/components/schemas/RhChainReviewedClassification' }, storage: rhChainClassificationStorage, authoritative: { const: true } }, ['classification', 'storage', 'authoritative']),
    RhChainClassificationAuditPayload: strictObject({ items: arrayOf({ $ref: '#/components/schemas/RhChainClassificationAuditEvent' }), page: { type: 'integer', minimum: 1 }, page_size: { type: 'integer', minimum: 1, maximum: 100 }, has_more: booleanSchema(), contract: rhChainExactContract, storage: rhChainClassificationStorage, authoritative: { const: true } }, ['items', 'page', 'page_size', 'has_more', 'contract', 'storage', 'authoritative']),
    RhChainPublicClassificationListPayload: strictObject({ classifications: arrayOf({ $ref: '#/components/schemas/RhChainPublicClassification' }), page: { type: 'integer', minimum: 1 }, page_size: { type: 'integer', minimum: 1, maximum: 100 }, has_more: booleanSchema(), authoritative: { const: true }, doctrine: { const: 'Exact contract identity and reviewed memory outrank provider context.' } }, ['classifications', 'page', 'page_size', 'has_more', 'authoritative', 'doctrine']),
    RhChainMarketProviderHealth: objectSchema({ provider: { const: 'dexscreener' }, enabled: booleanSchema(), healthy: booleanSchema(), degraded: booleanSchema(), disabled: booleanSchema(), state: enumSchema(['enabled', 'healthy', 'degraded', 'disabled']), lastSuccess: nullableString(), lastFailure: nullableString(), latestLatencyMs: { type: ['number', 'null'], minimum: 0 }, rollingFailureCount: { type: 'integer', minimum: 0 }, activeCacheStatus: enumSchema(['miss', 'fresh', 'stale_while_revalidate', 'stale_if_error', 'unavailable', 'disabled']), currentFreshness: enumSchema(['fresh', 'stale', 'unavailable', 'disabled']) }, ['provider', 'enabled', 'healthy', 'degraded', 'disabled', 'state', 'lastSuccess', 'lastFailure', 'latestLatencyMs', 'rollingFailureCount', 'activeCacheStatus', 'currentFreshness']),
    RhChainMarketPulseMetric: objectSchema({ value: { type: ['number', 'null'] }, previous_value: { type: ['number', 'null'] }, absolute_change: { type: ['number', 'null'] }, percentage_change: { type: ['number', 'null'] }, unit: enumSchema(['usd', 'count', 'percent', 'score']), provenance: arrayOf(stringSchema()), captured_at: dateTimeSchema(), freshness: enumSchema(['fresh', 'stale', 'partial', 'unavailable']), confidence: enumSchema(['high', 'medium', 'low']) }, ['value', 'previous_value', 'absolute_change', 'percentage_change', 'unit', 'provenance', 'captured_at', 'freshness', 'confidence']),
    RhChainMarketPulsePayload: objectSchema({ title: { const: 'Robinhood Chain Market Pulse' }, observation_window: objectSchema({ label: { const: 'Rolling 24 hours' }, current: freeformObject(), previous: freeformObject() }, ['label', 'current', 'previous']), metrics: freeformObject(), layer_composition: arrayOf(freeformObject()), momentum: freeformObject(), concentration: freeformObject(), interpretation: objectSchema({ headline: stringSchema(), conclusion: stringSchema(), supporting: arrayOf(stringSchema()), rules: arrayOf(stringSchema()), method: { const: 'deterministic_rules_v1' }, generated_at: dateTimeSchema(), freshness: enumSchema(['fresh', 'stale', 'partial', 'unavailable']), confidence: enumSchema(['high', 'medium', 'low']), provenance: arrayOf(stringSchema()) }, ['headline', 'conclusion', 'supporting', 'rules', 'method', 'generated_at', 'freshness', 'confidence', 'provenance']), provider_provenance: freeformObject(), captured_at: dateTimeSchema(), freshness: enumSchema(['fresh', 'stale', 'partial', 'unavailable']), warnings: arrayOf(stringSchema()), confidence: enumSchema(['high', 'medium', 'low']), data_mode: enumSchema(['live_cached', 'unavailable']), disclaimer: stringSchema() }, ['title', 'observation_window', 'metrics', 'layer_composition', 'momentum', 'concentration', 'interpretation', 'provider_provenance', 'captured_at', 'freshness', 'warnings', 'confidence', 'data_mode', 'disclaimer']),
    RhChainNormalizedMarketSnapshot: objectSchema({ provider: { const: 'dexscreener' }, chainId: { const: 'robinhood' }, capturedAt: dateTimeSchema(), tokenAddress: stringSchema(), baseToken: freeformObject(), quoteToken: freeformObject(), pairAddress: nullableString(), dexId: nullableString(), pairLabels: arrayOf(stringSchema()), priceUsd: { type: ['number', 'null'] }, liquidityUsd: { type: ['number', 'null'] }, fdv: { type: ['number', 'null'] }, marketCap: { type: ['number', 'null'] }, volume: freeformObject(), txns: freeformObject(), priceChange: freeformObject(), pairCreatedAt: nullableString(), activeBoosts: { type: 'number' }, paidOrders: arrayOf(freeformObject()), websites: arrayOf(freeformObject()), socials: arrayOf(freeformObject()), providerTimestamp: nullableString(), freshness: enumSchema(['fresh', 'stale']), rawDataVersion: { const: 'dexscreener-v1' }, cache: freeformObject(), sourceUrl: nullableString() }, ['provider', 'chainId', 'capturedAt', 'tokenAddress', 'pairAddress', 'dexId', 'priceUsd', 'liquidityUsd', 'fdv', 'marketCap', 'volume', 'txns', 'priceChange', 'pairCreatedAt', 'activeBoosts', 'paidOrders', 'sourceUrl']),
    RhChainMarketProviderStatusPayload: objectSchema({ provider: { const: 'dexscreener' }, enabled: booleanSchema(), chain_id: { const: 'robinhood' }, last_successful_capture: nullableString(), latest_boosts_observed: { type: 'integer', minimum: 0 }, known_token_refresh_state: enumSchema(['idle', 'fresh', 'partial_failure', 'fallback', 'disabled']), fallback_mode: booleanSchema(), health: { $ref: '#/components/schemas/RhChainMarketProviderHealth' }, caveats: arrayOf(stringSchema()) }, ['provider', 'enabled', 'chain_id', 'last_successful_capture', 'latest_boosts_observed', 'known_token_refresh_state', 'fallback_mode', 'health', 'caveats']),
    RhChainMarketTokenPayload: objectSchema({ token: freeformObject(), market_snapshot: { anyOf: [{ $ref: '#/components/schemas/RhChainNormalizedMarketSnapshot' }, { type: 'null' }] }, secondary_pairs: arrayOf({ $ref: '#/components/schemas/RhChainNormalizedMarketSnapshot' }), liquidity_fragmented: booleanSchema(), attention: freeformObject(), classification: freeformObject(), raw_provider_observation: freeformObject(), infopunks_analysis: freeformObject(), provenance: freeformObject(), caveats: arrayOf(stringSchema()) }, ['token', 'market_snapshot', 'secondary_pairs', 'liquidity_fragmented', 'attention', 'classification', 'raw_provider_observation', 'infopunks_analysis', 'provenance', 'caveats']),
    RhChainMarketTokenListPayload: objectSchema({ tokens: arrayOf({ $ref: '#/components/schemas/RhChainMarketTokenPayload' }), status: { $ref: '#/components/schemas/RhChainMarketProviderStatusPayload' } }, ['tokens', 'status']),
    RhChainMarketBoostsPayload: objectSchema({ boosts: arrayOf(freeformObject()), top_boosts: arrayOf(freeformObject()), status: { $ref: '#/components/schemas/RhChainMarketProviderStatusPayload' }, caveats: arrayOf(stringSchema()) }, ['boosts', 'top_boosts', 'status', 'caveats']),
    RhChainMarketSnapshotCapturePayload: objectSchema({ captured: arrayOf(freeformObject()), status: stringSchema(), storage: freeformObject(), caveats: arrayOf(stringSchema()) }, ['captured', 'status', 'caveats']),
    RhChainMarketSnapshotListPayload: objectSchema({ token: freeformObject(), snapshots: arrayOf(freeformObject()), snapshot_count: { type: 'integer' }, caveats: arrayOf(stringSchema()) }, ['token', 'snapshots', 'snapshot_count', 'caveats']),
    RhChainAttentionQualityHistoryPayload: objectSchema({ contract: stringSchema(), snapshot_count: { type: 'integer' }, state: stringSchema(), score: { anyOf: [{ type: 'number' }, { type: 'null' }] }, components: freeformObject(), caveats: arrayOf(stringSchema()), assessment_state: stringSchema(), attention_quality_score: { anyOf: [{ type: 'number' }, { type: 'null' }] }, provider_requests_in_path: { type: 'integer', const: 0 } }, ['contract']),
    RhChainMarketStructurePayload: objectSchema({ title: { const: '4663 Market Structure' }, market_pulse: freeformObject(), market_composition: arrayOf(freeformObject()), layer_leaders: arrayOf(freeformObject()), observed_coverage: freeformObject() }, ['title', 'market_pulse', 'market_composition', 'layer_leaders', 'observed_coverage']),
    RhChainCrossLayerPayload: objectSchema({ title: { const: 'Cross-Layer Assets' }, integration_enabled: booleanSchema(), headline: stringSchema(), interpretation: freeformObject(), observation_window: freeformObject(), entries: arrayOf(freeformObject()), categories: arrayOf(stringSchema()), intersection_count: { type: 'integer', minimum: 0 }, reviewed_project_count: { type: 'integer', minimum: 0 }, intersection_counts: arrayOf(freeformObject()), layer_pair_counts: arrayOf(freeformObject()), tracked_liquidity_by_intersection: arrayOf(freeformObject()), tracked_volume_by_intersection: arrayOf(freeformObject()), concentration: freeformObject(), classification_coverage: freeformObject(), market_data_coverage: freeformObject(), conflict_count: { type: 'integer', minimum: 0 }, source_required_count: { type: 'integer', minimum: 0 }, unknown_count: { type: 'integer', minimum: 0 }, captured_at: dateTimeSchema(), freshness: enumSchema(['fresh', 'stale', 'partial', 'unavailable']), confidence: enumSchema(['high', 'medium', 'low']), warnings: arrayOf(stringSchema()), provider_provenance: freeformObject(), classification_provenance: freeformObject(), bounded_universe: stringSchema(), methodology_version: stringSchema(), data_mode: stringSchema(), caveats: arrayOf(stringSchema()), observed_at: dateTimeSchema() }, ['title', 'entries', 'categories', 'observed_at']),
    RhChainCrossLayerConflictPayload: strictObject({ captured_at: dateTimeSchema(), conflict_count: { type: 'integer', minimum: 0, maximum: 100 }, conflicts: { type: 'array', maxItems: 100, items: freeformObject() }, source_required_count: { type: 'integer', minimum: 0, maximum: 100 }, source_required: { type: 'array', maxItems: 100, items: freeformObject() }, warnings: arrayOf(stringSchema()), bounded_universe: stringSchema(), classification_precedence: { type: 'array', items: stringSchema(), minItems: 4, maxItems: 4 }, resolution_policy: stringSchema() }, ['captured_at', 'conflict_count', 'conflicts', 'source_required_count', 'source_required', 'warnings', 'bounded_universe', 'classification_precedence', 'resolution_policy']),
    RhChainAttentionQualityPayload: objectSchema({ title: stringSchema(), observations: arrayOf(freeformObject()), attention_quality: arrayOf(freeformObject()), observed_at: dateTimeSchema(), provider_requests_in_path: { type: 'integer', const: 0 }, methodology_version: stringSchema() }, ['title', 'observations', 'attention_quality']),
    RhChainAttentionQualityReceiptPayload: objectSchema({ receipt_id: stringSchema(), status: enumSchema(['draft', 'published', 'rejected']), schema_version: { type: 'integer', const: 1 }, assessment: freeformObject(), created_at: dateTimeSchema() }, ['receipt_id', 'status', 'schema_version', 'assessment', 'created_at']),
    ...
    RhChainProjectClaimSubmissionRequest: objectSchema({ project_name: stringSchema(), chain: { const: 'robinhood' }, exact_contract: stringSchema(), contact: stringSchema(), claim_category: stringSchema(), claim_statement: stringSchema(), supporting_evidence: arrayOf(stringSchema()), disclosure_accepted: { const: true }, authority_to_submit_confirmed: { const: true }, privacy_consent: booleanSchema() }, ['project_name', 'chain', 'exact_contract', 'contact', 'claim_category', 'claim_statement', 'supporting_evidence', 'disclosure_accepted', 'authority_to_submit_confirmed']),
    RhChainProjectDirectoryPayload: objectSchema({ items: arrayOf(freeformObject()), next_cursor: nullableString() }, ['items', 'next_cursor']),
    RhChainProjectPayload: freeformObject(),
    RhChainProjectClaimsPayload: objectSchema({ project_id: stringSchema(), project_submitted_claims: arrayOf(freeformObject()), not_financial_advice: booleanSchema() }, ['project_id', 'project_submitted_claims', 'not_financial_advice']),
    RhChainProjectObservationsPayload: objectSchema({ project_id: stringSchema(), infopunks_observations: arrayOf(freeformObject()), onchain_evidence: arrayOf(freeformObject()), not_financial_advice: booleanSchema() }, ['project_id', 'infopunks_observations', 'onchain_evidence', 'not_financial_advice']),
    RhChainProjectReceiptsPayload: objectSchema({ project_id: stringSchema(), receipts: arrayOf(freeformObject()), not_financial_advice: booleanSchema() }, ['project_id', 'receipts', 'not_financial_advice']),
    RhChainProjectSubmissionResponse: objectSchema({ status: { const: 'queued_for_review' }, no_endorsement: { const: true }, no_automatic_publication: { const: true }, no_automatic_classification: { const: true }, submission_id: stringSchema(), submitted_timestamp: dateTimeSchema(), next_review_state: stringSchema() }, ['status', 'no_endorsement', 'no_automatic_publication', 'no_automatic_classification', 'submission_id']),
    RhChainIntelligenceReceiptListPayload: objectSchema({ receipts: arrayOf({ $ref: '#/components/schemas/RhChainIntelligenceReceipt' }), not_financial_advice: booleanSchema() }, ['receipts', 'not_financial_advice']),
    RhChainIntelligenceReceiptPayload: objectSchema({ receipt: { $ref: '#/components/schemas/RhChainIntelligenceReceipt' }, share: freeformObject(), not_financial_advice: booleanSchema() }, ['receipt', 'not_financial_advice']),
    RhChainIntelligenceReceipt: objectSchema({ receipt_id: stringSchema(), exact_contract_identity: stringSchema(), verdict: stringSchema(), integrity_hash: stringSchema(), reviewer_publication_state: enumSchema(['published', 'superseded']), not_financial_advice: { const: true }, correction_history: arrayOf(freeformObject()) }, ['receipt_id', 'exact_contract_identity', 'verdict', 'integrity_hash', 'reviewer_publication_state', 'not_financial_advice']),
