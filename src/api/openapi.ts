...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/frontdoor', rh4663('Get the 4663 Front Door read model', 'Returns the compact, server-assembled consumer snapshot for /4663. It references canonical Census, Watch, Preflight, Signal, and Pulse objects but is not canonical evidence itself. `frontdoor_version` changes only when the visible derived source state changes, so clients can cheaply revalidate without diffing the full snapshot. It uses latest persisted reads only, exposes per-source health, and may return partial state when a module is unavailable.', 'Rh4663FrontdoorState', { object_type: 'RH_4663_FRONTDOOR_STATE', generated_at: '2026-09-03T12:00:00.000Z', frontdoor_version: { object_type: 'FRONTDOOR_VERSION', version: 1821, changed: ['RMM_CENSUS', 'AI_NVDA_CASE', 'PULSE'], generated_at: '2026-09-03T12:00:00.000Z' }, now_cards: [], watch_cards: [], open_loops: [], system_status: { state: 'partial', source_health: { census: { status: 'available' } } } }));
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  return {
    ...
    Rh4663FrontdoorState: objectSchema({ object_type: { const: 'RH_4663_FRONTDOOR_STATE' }, generated_at: dateTimeSchema(), freshness: objectSchema({ state: enumSchema(['VERIFIED', 'MIXED', 'WATCH', 'UNRESOLVED', 'BLOCK', 'DEGRADE', 'INSUFFICIENT_DATA']), source_observed_at: nullableString() }, ['state', 'source_observed_at']), frontdoor_version: objectSchema({ object_type: { const: 'FRONTDOOR_VERSION' }, version: { type: 'integer', minimum: 1 }, changed: arrayOf(stringSchema()), generated_at: dateTimeSchema() }, ['object_type', 'version', 'changed', 'generated_at']), now_cards: arrayOf(freeformObject()), watch_cards: arrayOf(freeformObject()), open_loops: arrayOf(freeformObject()), current_call: freeformObject(), proof_summary: freeformObject(), system_status: freeformObject(), source_refs: arrayOf(freeformObject()) }, ['object_type', 'generated_at', 'freshness', 'frontdoor_version', 'now_cards', 'watch_cards', 'open_loops', 'current_call', 'proof_summary', 'system_status', 'source_refs']),
