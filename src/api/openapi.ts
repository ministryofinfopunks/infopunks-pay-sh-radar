...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/print-candidate', rh4663('Generate a 4663 PRINT candidate', 'Normalizes existing cached provider observations into a review-only candidate. It shows source windows, gaps, freshness, and disagreements; it never publishes a PRINT.', 'Rh4663PrintCandidate', { candidate_id: 'rh-print-candidate-2026-08-31', lifecycle: 'CANDIDATE', warnings: ['Missing required verified observation: transactions_utc_day.'] }, undefined, [{ name: 'date', in: 'query', required: false, schema: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' } }]));
