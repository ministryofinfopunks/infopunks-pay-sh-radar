...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/rh-chain/intelligence-receipts/{receipt_id}', rhChain('Get immutable Intelligence Receipt', 'Returns a published or superseded receipt with its integrity hash and correction linkage.', 'RhChainIntelligenceReceiptPayload', { receipt: { receipt_id: 'rhir_example', integrity_hash: 'sha256:example', not_financial_advice: true }, not_financial_advice: true }, [pathParam('receipt_id', 'Immutable receipt identifier.')]));
...
function componentSchemas(): Record<string, JsonSchema> {
  ...
  return {
    ...
    RhChainIntelligenceReceiptPayload: objectSchema({ receipt: { $ref: '#/components/schemas/RhChainIntelligenceReceipt' }, not_financial_advice: booleanSchema() }, ['receipt', 'not_financial_advice']),
