...
export function createOpenApiSpec(version = '0.1.0'): OpenApiSpec {
  ...
  add('get', '/v1/4663/reflexive/pairs', rh4663('List verified Stock-quoted mission pairs', 'Lists only pairs whose quote contract matches the canonical Robinhood chain-4663 asset registry; ticker matches alone are rejected.', 'ReflexivePairList', { pairs: [], methodology_version: 'rmm-v0.1.0' }));
  add('get', '/v1/4663/reflexive/pairs/{id}', rh4663('Get mission-pair dossier', 'Returns one verified pair, immutable observations, and explicit provenance. V4 singleton PoolManager balances are never used as pair inventory.', 'ReflexivePairDossier', { pair: { canonicality: 'verified', inventory_method: 'unavailable' } }, undefined, [pathParam('id', 'Deterministic mission-pair identifier.')]));
  add('get', '/v1/4663/reflexive/stocks/{symbol}', rh4663('Get Stock Token quote-economy dossier', 'Returns canonical token, tracked mission pairs, supply events, and separate gross-volume/inventory concepts. Net quote demand is unavailable until attributable.', 'ReflexiveStockDossier', { asset: { ticker: 'NVDA' }, net_quote_demand: 'NOT_YET_ATTRIBUTABLE' }, undefined, [pathParam('symbol', 'Canonical Stock Token ticker.')]));
