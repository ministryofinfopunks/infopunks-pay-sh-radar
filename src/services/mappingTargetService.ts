...
const mappingTargets: MappingTarget[] = [
  ...
  },
  {
    ...
    current_state: 'needs_two_comparable_mappings',
    needed_next_step: 'Benchmark Scaffold: keep StableEnrich as the only paid-proven route (degraded evidence), then find another comparable place-search provider route or revisit Google Places only if provider schema/output changes on canonical input {"query":"coffee near Union Square San Francisco","location":"Union Square, San Francisco, CA","limit":5}, then record one five-run benchmark artifact.',
    ...
    readiness_blocker: 'Scaffold only: StableEnrich Google Maps Text Search is paid-proven for recognizable place-search semantics but evidence is degraded (missing names/addresses/coordinates and unconfirmed location), Google Places SearchText paid execution and one paid diagnostic retry (includedType=cafe) still returned zero recognizable place candidates, no second comparable paid-proven route is recorded on the same canonical input, and no five-run benchmark artifact exists.'
  }
];
