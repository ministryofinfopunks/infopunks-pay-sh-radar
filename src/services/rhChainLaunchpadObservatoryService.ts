...
export function assembleRhChainLaunchpadObservatory(): RhChainLaunchpadObservatoryPayload {
  const surfaces: RhChainLaunchpadSurface[] = [
    surface({ surface_id: 'noxa_fun', name: 'NOXA Fun', status: 'degraded', status_confidence: 'medium', source_notes: ['Reported NOXA launch pause and fee-model mutation are retained as manual context.', 'Current availability, domain state, and fee/revenue details require primary, timestamped or on-chain evidence.'], known_tokens: [], notable_claims: ['Reported disruption is not a finding about intent, solvency, misconduct, or safety.', 'Reported creator-revenue shift remains source_required until primary/on-chain evidence exists.'], fee_claims: ['Fee terms are source_required.', 'Reported creator revenue redirect is source_required.'], launch_count_claims: ['Launch-count continuity is source_required.'], risk_notes: ['A front-end can become a single point of origin confusion.', 'Fee-model mutation can create creator-fee confusion without primary terms.'], surface_risks: ['front_end_dependency', 'creator_fee_claim_uncertainty', 'fee_model_mutation', 'source_required'], infopunks_note: 'The stress event matters because memory must outlast one interface and one fee design.' }),
    ...
    surface({ surface_id: 'uniswap_direct_launches', name: 'Uniswap direct launches', status: 'active', status_confidence: 'medium', source_notes: ['Direct-pool creation is observable launch context, not a quality, liquidity, or safety determination.'], known_tokens: [], notable_claims: ['A direct pool does not establish a canonical launch origin by itself.'], fee_claims: ['Pool, creator-fee, burn, and buyback claims require exact, dated evidence.'], launch_count_claims: ['No aggregate launch count is promoted without a reviewed method.'], risk_notes: ['Low-quality direct pools can increase origin and liquidity ambiguity.'], surface_risks: ['launch_quality_filtering', 'fee_model_mutation', 'source_required'], infopunks_note: 'A pool is evidence to inspect, not a verdict.' }),
  ...
  return {
    ...
    claim_ledger: [
      ...
      claim('noxa-fee-model-shift', 'fee_claim', 'noxa_fun', 'Reported NOXA fee-model shift during the launch pause.'),
      claim('noxa-creator-revenue-shift', 'creator_revenue_shift', 'noxa_fun', 'Reported shift toward redirecting transaction revenue to creators.'),
    ...
    post_noxa_stress_map: [
      ...
      { title: 'Fee design becomes a battlefield', explanation: 'Reported creator-revenue shifts, fee routing, burns, and buybacks remain source_required unless primary or on-chain evidence is attached.' },
      { title: 'Fragmentation raises the memory requirement', explanation: 'More launch surfaces mean more opportunities for origin confusion, duplicate claims, fee confusion, and stale screenshots. Exact contracts, sources, and reviewed receipts matter more.' }
    ],
    risk_notes: [
      ...
      { title: 'Fee-model claim laundering', explanation: 'Creator-fee, burn, buyback, and transaction-revenue claims need primary terms or on-chain evidence before promotion.' },
      ...
      { title: 'Rival-pad impersonation', explanation: 'Rival-surface branding or screenshots do not establish canonical launch origin.' }
    ]
  };
}
