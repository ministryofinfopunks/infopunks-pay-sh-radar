...
export type RhChainTokenPairSnapshot = { contract: string; exact_contract_match: boolean; pair_address: string | null; dex_url: string | null; liquidity_usd: number | null; volume_24h_usd: number | null; fdv_usd: number | null; market_cap_usd: number | null; pair_created_at: string | null; source_timestamp: string | null; freshness: RhChainSnapshotFreshness };
export type RhChainExplorerSnapshot = { exact_contract_match: boolean; explorer_url: string | null; contract_verified: boolean | null; deployer_address: string | null; source_timestamp: string | null; freshness: RhChainSnapshotFreshness };
...
export type RhChainLiveProviderClient = {
  ...
  tokenPair(contract: string): Promise<Omit<RhChainTokenPairSnapshot, 'contract' | 'freshness' | 'exact_contract_match'> & { observed_contract?: string | null }>;
  explorer(contract: string): Promise<Omit<RhChainExplorerSnapshot, 'freshness' | 'exact_contract_match'> & { observed_contract?: string | null }>;
};
...
export function normalizeRhChainTokenPair(contract: string, value: Awaited<ReturnType<RhChainLiveProviderClient['tokenPair']>>, status: RhChainSnapshotStatus, now: Date): RhChainTokenPairSnapshot {
  return { contract, exact_contract_match: isExactRhChainContractMatch(contract, value.observed_contract), pair_address: value.pair_address ?? null, dex_url: value.dex_url ?? null, liquidity_usd: finiteNumber(value.liquidity_usd), volume_24h_usd: finiteNumber(value.volume_24h_usd), fdv_usd: finiteNumber(value.fdv_usd), market_cap_usd: finiteNumber(value.market_cap_usd), pair_created_at: validateRhChainSourceTimestamp(value.pair_created_at, now), source_timestamp: validateRhChainSourceTimestamp(value.source_timestamp, now), freshness: snapshotFreshness(status) };
}
export function normalizeRhChainExplorer(contract: string, value: Awaited<ReturnType<RhChainLiveProviderClient['explorer']>>, status: RhChainSnapshotStatus, now: Date): RhChainExplorerSnapshot {
  return { exact_contract_match: isExactRhChainContractMatch(contract, value.observed_contract), explorer_url: value.explorer_url ?? null, contract_verified: value.contract_verified ?? null, deployer_address: value.deployer_address ?? null, source_timestamp: validateRhChainSourceTimestamp(value.source_timestamp, now), freshness: snapshotFreshness(status) };
}
...
function inferLaunchContext(pair: RhChainTokenPairSnapshot | null, explorer: RhChainExplorerSnapshot | null): RhChainLaunchContext | undefined {
  ...
  return { launch_source: 'unknown_manual', launch_source_type: 'unknown_manual', launch_surface_url: pair?.exact_contract_match ? pair.dex_url : null, contract_verified: explorer?.exact_contract_match ? explorer.contract_verified ?? 'unknown' : 'unknown', liquidity_route: pair?.exact_contract_match && pair.dex_url ? 'provider-observed DEX route' : null, pair_address: pair?.exact_contract_match ? pair.pair_address : null, lp_status: 'unknown', deployer_address: explorer?.exact_contract_match ? explorer.deployer_address : null, creator_address: null, deployer_observed_at: explorer?.exact_contract_match && explorer.deployer_address ? observed_at : null, source_notes: 'Provider context is not identity proof unless the provider returned the exact requested contract. Launch surface remains unknown without a human-reviewed receipt.', evidence_links: [], confidence_level: 'low', data_mode: 'cached', observed_at, updated_at: observed_at };
}
...
function createPublicClients(options: RhChainLiveSnapshotOptions): RhChainLiveProviderClient {
  ...
  return {
    ...
    async tokenPair(contract) { const pairs = await request<Array<{ pairAddress?: string; url?: string; baseToken?: { address?: string }; quoteToken?: { address?: string }; liquidity?: { usd?: number }; volume?: { h24?: number }; fdv?: number; marketCap?: number; pairCreatedAt?: number }>>(`https://api.dexscreener.com/latest/dex/tokens/${encodeURIComponent(contract)}`); const pair = pairs.find((item) => isExactRhChainContractMatch(contract, item.baseToken?.address) || isExactRhChainContractMatch(contract, item.quoteToken?.address)); if (!pair) throw new Error('provider_contract_mismatch'); const observed_contract = isExactRhChainContractMatch(contract, pair.baseToken?.address) ? pair.baseToken?.address : pair.quoteToken?.address; return { observed_contract, pair_address: pair.pairAddress ?? null, dex_url: pair.url ?? null, liquidity_usd: pair.liquidity?.usd ?? null, volume_24h_usd: pair.volume?.h24 ?? null, fdv_usd: pair.fdv ?? null, market_cap_usd: pair.marketCap ?? null, pair_created_at: pair.pairCreatedAt ? new Date(pair.pairCreatedAt).toISOString() : null, source_timestamp: new Date().toISOString() }; },
    async explorer(contract) { if (!options.blockscoutUrl) throw new Error('blockscout_endpoint_not_configured'); return { observed_contract: contract, explorer_url: `${options.blockscoutUrl.replace(/\/$/, '')}/address/${encodeURIComponent(contract)}`, contract_verified: null, deployer_address: null, source_timestamp: new Date().toISOString() }; }
  };
}
