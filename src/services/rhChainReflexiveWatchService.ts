...
import { buildAiNvdaD7ReAudit, type AiNvdaD7ReAudit } from './aiNvdaD7ReAuditService';
...
export type RwaActivationCase = {
  ...
  research_observations: AiNvdaD7ReAudit[];
...
export class ReflexiveMarketsWatchService {
  ...
  async snapshot(): Promise<ReflexiveMarketsWatchSnapshot> {
    ...
    const cases = buildSeedCases(radar, claims, this.now());
...
function buildSeedCases(radar: ReflexiveSnapshot, claims: ReflexiveWatchClaim[], now: Date): RwaActivationCase[] {
  ...
  const aiD7 = buildAiNvdaD7ReAudit(radar, now);
  const aiDimensions = dimensions({ ATTENTION: 'SUPPORTING_EVIDENCE', STOCK_TOKEN_INVENTORY: aiEvidence.some((item) => item.radar_state === 'VERIFIED') ? 'CONFIRMED' : 'SUPPORTING_EVIDENCE', CAPITAL_PERSISTENCE: aiD7.h2b_verdict === 'FALSIFYING_EVIDENCE' ? 'FALSIFYING_EVIDENCE' : 'SUPPORTING_EVIDENCE', FLOW_PERSISTENCE: aiD7.flow_regime === 'MULTIRAIL_FLOW' ? 'MIXED' : aiD7.h2b_verdict === 'FALSIFYING_EVIDENCE' ? 'FALSIFYING_EVIDENCE' : 'MIXED', QUOTE_PERSISTENCE: aiD7.capital_vs_flow_regime === 'INSUFFICIENT_DATA' ? 'MIXED' : 'SUPPORTING_EVIDENCE', SUPPLY_RESPONSE_SEQUENCE: radar.supply_events.some((item) => item.asset_id === radar.assets.find((asset) => asset.ticker === 'NVDA')?.asset_id) ? 'SUPPORTING_EVIDENCE' : 'UNVERIFIED', BASIS_BEHAVIOR: 'UNAVAILABLE', FEE_ACTIVITY: 'UNVERIFIED', TREASURY_OR_VAULT_ACCUMULATION: 'UNAVAILABLE', ROUTE_EXPANSION: 'SUPPORTING_EVIDENCE', SURVIVAL: aiD7.h2b_verdict === 'FALSIFYING_EVIDENCE' ? 'FALSIFYING_EVIDENCE' : aiD7.h2b_verdict === 'SUPPORTING_EVIDENCE' ? 'SUPPORTING_EVIDENCE' : 'SUPPORTING_EVIDENCE' });
  return [
    rwaCase({ case_id: 'AI_NVDA_CAPITAL_VS_FLOW', title: 'AI / NVDA Capital vs Flow', mission_assets: ['AI'], rwa_or_stock_token: ['NVDA'], venues: ['LONG', 'Doppler', 'PAIR', 'Uniswap v4'], opening_hypothesis: 'Crypto-native AI market activity coincided with material canonical NVDA Stock Token usage, but capital, flow and attribution must remain separate.', opened_at: '2026-09-01T00:00:00.000Z', watch_claims: aiClaimIds, radar_evidence: aiEvidence, research_observations: [aiD7], persistence_observations: persistenceFromRadar(radar, aiD7), activation_dimensions: aiDimensions, current_evidence_state: aiD7.h2b_verdict === 'FALSIFYING_EVIDENCE' ? 'FALSIFIED' : aiD7.h2b_verdict === 'SUPPORTING_EVIDENCE' ? 'PARTIALLY_VERIFIED' : 'MIXED', open_evidence_gaps: ['Community vault identity unavailable.', 'External liquidity is possible but not included in verified footprint.', 'Synchronized underlying/NVDA basis remains unavailable.', 'D7-D14 minimum READY shadow evidence window is separate and still controls v0.5.2.'], candidate_next_audit: aiD7.status === 'OBSERVED' ? 'Re-run AI/NVDA at D30 with the same frozen retention, absorption and quote-share split.' : 'Re-run AI/NVDA at D7 with same-block launch-position inventory, totalSupply and quote topology.', priority: deterministicAuditPriority({ material_stock_share_claim: true, new_architecture: false, strong_falsification_potential: true, cross_market_significance: true, historical_data_urgency: false, existing_radar_adapter: true }), activity_classification: classifyActivationEvidence(aiDimensions), thesis_mapping: ['H2A', 'H2B', 'H2C', 'H2D', 'H7', 'RA1'], falsification_notes: aiD7.h2b_verdict === 'FALSIFYING_EVIDENCE' ? ['D7 principal retention and stock quote share crossed the precommitted falsification band.'] : ['Stock quote flow persistence is mixed and can challenge stock quote execution dominance.'] }),
    ...
    rwaCase({ case_id: 'RH_STOCK_MEME_MAP_20260903', title: 'RH Stock / Meme Map Census', mission_assets: ['MULTIPLE'], rwa_or_stock_token: ['MULTIPLE'], venues: ['UNKNOWN'], opening_hypothesis: 'Watch reports approximately 216 Stock Tokens, 27 direct meme/stock pairs and 22 underlying tickers. Radar counts the active canonical denominator and verifies exact identity plus quote direction before any category conclusion.', opened_at: '2026-09-03T00:00:00.000Z', watch_claims: claims.filter((item) => item.claim_text.includes('approximately 216 Stock Tokens')).map((item) => item.claim_id), radar_evidence: [{ evidence_id: 'radar:rmm-category-census', label: 'RMM Category Census', href: '/v1/4663/reflexive/census', status: 'RADAR_PARTIAL', radar_state: 'PARTIAL', note: 'Watch said 27 / 22. Radar preserves the parsed and verified differences instead of normalizing the source.' }], persistence_observations: unavailablePersistence(), activation_dimensions: dimensions({ ATTENTION: 'UNVERIFIED', STOCK_TOKEN_INVENTORY: 'UNVERIFIED', QUOTE_PERSISTENCE: 'UNVERIFIED', SURVIVAL: 'UNAVAILABLE' }), current_evidence_state: 'VERIFYING', open_evidence_gaps: ['Two claimed pairs and one ticker are absent from the supplied examples.', 'A pool is not counted until the mission quote direction is deterministic.', 'Breadth cannot confirm RA1 persistence.'], candidate_next_audit: 'Refresh the immutable RMM category census; select only the ranked deep-audit queue after coverage is visible.', priority: 'HIGH', activity_classification: 'SPECULATIVE_ACTIVITY', thesis_mapping: ['RA1'], falsification_notes: [] }),
...
function persistenceFromRadar(radar: ReflexiveSnapshot, d7Audit?: AiNvdaD7ReAudit): PersistenceObservation[] {
  ...
  return checkpoints.map((checkpoint) => {
    ...
    if (checkpoint === 'D7' && d7Audit) return { checkpoint, state: d7Audit.status === 'OBSERVED' ? 'OBSERVED' : d7Audit.status === 'PENDING' ? 'PENDING' : 'UNAVAILABLE', observed_at: d7Audit.d7?.timestamp ?? null, stock_token_footprint_delta: d7Audit.change.absorption_change_percentage_points, capital_share_delta: d7Audit.change.liquidity_share_change_percentage_points, flow_share_delta: d7Audit.change.volume_share_change_percentage_points, supply_delta: d7Audit.change.nvda_supply_change, basis_context: d7Audit.d7?.basis_session_context.stock_token_basis ?? null, survival_state: d7Audit.h2b_verdict };
...
function rwaCase(input: Omit<RwaActivationCase, 'object_type' | 'research_observations' | 'audit_priority' | 'priority_rationale' | 'state_transitions' | 'updated_at' | 'methodology_version' | 'immutable'> & { priority: AuditPriority; research_observations?: AiNvdaD7ReAudit[] }): RwaActivationCase {
  return deepFreeze({ ...input, research_observations: input.research_observations ?? [], object_type: 'RWA_ACTIVATION_CASE', audit_priority: input.priority, priority_rationale: ['Deterministic research priority excludes token price upside and market-cap potential.'], state_transitions: [{ from: null, to: input.current_evidence_state, transitioned_at: input.opened_at, reason: input.opening_hypothesis }], updated_at: input.opened_at, methodology_version: REFLEXIVE_WATCH_METHOD_VERSION, immutable: true });
}
...
function seedWatchClaims(): ReflexiveWatchClaim[] {
  ...
  return [
    ...
    , createWatchClaim({ ...base, captured_at: '2026-09-03T00:00:00.000Z', source_type: 'THIRD_PARTY_RESEARCH', source_name: 'Watch RMM category map', claim_text: 'Approximately 216 Stock Tokens, approximately 27 direct meme/Stock Token pairs, and approximately 22 underlying tickers exist on Robinhood Chain.', subject_assets: ['RMM_CATEGORY'], mission_token: null, stock_token_or_rwa: null, venue: null, claimed_metrics: { claimed_canonical_stock_tokens: '216', claimed_direct_pairs: '27', claimed_distinct_stock_tickers: '22' }, claimed_timestamp: '2026-09-03T00:00:00.000Z', claim_category: 'OTHER', provisional_interpretation: 'Category-level supporting claim. Radar must establish the denominator, parse reconciliation, canonical identity and deterministic quote direction.', watch_verdict: 'INSUFFICIENT_DATA', evidence_status: 'RADAR_AUDIT_TARGET' })
  ];
}
