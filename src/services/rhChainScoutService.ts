...
export function queryRhChainScout(input: RhChainScoutQuery, reviewItems = getRhChainReviewQueue().items, observatory?: RhChainLaunchpadObservatoryPayload): RhChainScoutResponse {
  ...
  const narrativeAnswer = /what does robinhood agentic trading change|agentic trading change/.test(needle)
    ? `Robinhood Agentic Trading turns the agent layer into documented platform infrastructure: a third-party AI agent can connect through Robinhood’s Trading MCP to a dedicated Agentic Account with a dedicated budget, visible activity, notifications, and a disconnect control. Robinhood currently documents long equities and options orders; crypto support remains source_required unless exact current primary documentation verifies it. This is a Robinhood platform event, not proof of RH Chain agent adoption.`
    : /why.*agent.*layer.*gaining power|agent.*layer.*gaining power/.test(needle)
      ? `The agent layer is gaining power because Robinhood now provides documented platform infrastructure instead of only an AI narrative: MCP integration, a dedicated account boundary, and a documented brokerage order path. The platform-level infrastructure signal is medium confidence; RH Chain-specific adoption remains low confidence until operational receipts exist.`
      : /does.*agentic trading.*prove.*rh chain.*adoption|agentic trading.*prove.*adoption/.test(needle)
        ? `No. Agentic Trading proves a Robinhood brokerage product surface, not proof of RH Chain agent adoption. MCP access, account setup, activity visibility, or brokerage execution do not establish on-chain wallets, RWAs, DeFi routes, tokenized-asset activity, or capital migration. Those remain under_receipt_check until source-linked operational receipts exist.`
        : /bridge.*brokerage agents.*rh chain|brokerage agents.*rh chain/.test(needle)
          ? `The possible bridge is: agent research and brokerage execution inside a dedicated Agentic Account, then demonstrated interest or routing into tokenized assets, RWAs, DeFi, prediction or event markets, and infrastructure contracts. It is a monitoring thesis, not a verified flow. 4663 needs primary route documentation, snapshots, wallet-flow evidence, and reviewed receipts before calling it RH Chain adoption.`
          : /what should 4663 track next.*agents|track next.*agents/.test(needle)
            ? `4663 should track, as source_required: brokerage-agent to tokenized-stock interest; agent research to RH Chain RWA activity; agent-managed portfolio to on-chain DeFi route; meme liquidity to agent strategy; agent infrastructure to RWA execution; and MCP tooling to RH Chain contract interaction. The test is operational cross-layer flow, not product visibility.`
            : /difference.*agent visibility.*agent usage|agent visibility.*agent usage/.test(needle)
              ? `Agent visibility is a documented surface: an MCP endpoint, Agentic Account, activity feed, or product announcement. Agent usage is evidence that agents actually act through it, ideally with source-linked activity records. Neither alone proves RH Chain adoption; that needs an on-chain route, wallet or contract activity where observable, and a reviewed receipt.`
  : /which.*layer.*gaining power/.test(needle)
