...
function claimedLaunchContext(input: RhChainSignalSubmissionInput, observedAt: string): RhChainLaunchContext | undefined {
  ...
  const launchSource = input.launch_source ?? 'unknown_manual';
  const launchpads = ['noxa_fun', 'flap_sh', 'trensh_today', 'bankr', 'tokeny_fun', 'vlad_fun', 'robindotmarket'];
  return { launch_source: launchSource, launch_source_type: launchpads.includes(launchSource) ? 'launchpad' : launchSource === '20lab_erc20' ? 'token_generator' : launchSource === 'pump_fun_routed_rh_chain' ? 'routed_launchpad' : launchSource === 'uniswap_direct_pool' ? 'direct_dex_pool' : launchSource === 'hardhat_foundry_custom' ? 'custom_deployment' : 'unknown_manual', launch_surface_url: optional(input.launch_surface_url), contract_verified: 'unknown', liquidity_route: input.pair_address ? 'submitter-claimed pair route' : null, pair_address: optional(input.pair_address), lp_status: input.lp_status_claim ?? 'unknown', deployer_address: optional(input.deployer_address), creator_address: null, deployer_observed_at: input.deployer_address ? observedAt : null, source_notes: 'Submitter-provided launch context. Unverified until human receipt review.', evidence_links: urls.map((url, index) => ({ label: index === 0 ? 'Submitter launch context' : `Submitter evidence ${index}`, url, note: 'External, untrusted submitter-provided link. Claim only; not an approval or safety determination.', observed_at: observedAt })), confidence_level: 'low', data_mode: 'community_submission', observed_at: observedAt, updated_at: observedAt };
}
