...
type CardProps = { topic: string; conclusion: string; metric: string; delta?: string; evidence?: string; freshness?: string | null; href: string; shareId?: string } & FollowProps;
function FrontdoorShare({ shareId }: { shareId?: string }) { if (!shareId) return null; const [status, setStatus] = useState(''); const href = `/4663/share/${encodeURIComponent(shareId)}`; async function share() { frontdoorEvent('share_clicked', shareId); try { const url = new URL(href, window.location.origin).toString(); if (navigator.share) { await navigator.share({ title: '//4663 evidence', text: 'A source-bound Infopunks //4663 finding.', url }); frontdoorEvent('share_native_completed', shareId); } else { await navigator.clipboard.writeText(url); frontdoorEvent('share_link_copied', shareId); setStatus('Link copied.'); } } catch { /* native-sheet cancellation leaves the finding untouched */ } } return <><button className="fd-share" type="button" onClick={share} aria-label="Share this evidence">SHARE</button><span className="sr-only" role="status" aria-live="polite">{status}</span></>; }
export function NowCard(props: CardProps) { return <article className="fd-card fd-now-card"><p>{props.topic}</p><h3>{props.conclusion}</h3><MetricDelta metric={props.metric} delta={props.delta} /><footer><EvidenceBadge state={props.evidence} /><FreshnessBadge at={props.freshness} /><FollowControl follow={props.follow} following={props.following} onToggleFollow={props.onToggleFollow} /><FrontdoorShare shareId={props.shareId} /><ResearchLink href={props.href}>Dossier</ResearchLink></footer></article>; }
export function WatchCard(props: CardProps) { return <article className="fd-card fd-watch-card"><p>{props.topic}</p><h3>{props.conclusion}</h3><footer><EvidenceBadge state={props.evidence ?? 'WATCH'} /><FreshnessBadge at={props.freshness} /><FollowControl follow={props.follow} following={props.following} onToggleFollow={props.onToggleFollow} /><FrontdoorShare shareId={props.shareId} /><ResearchLink href={props.href}>Case</ResearchLink></footer></article>; }
function frontdoorEvent(event: 'open_loop_viewed' | 'open_loop_opened' | 'open_loop_source_opened' | 'return_change_summary_viewed' | 'return_change_opened' | 'pending_call_changes_viewed' | 'resolved_call_return_viewed' | 'frontdoor_return_visit' | 'follow_created' | 'follow_removed' | 'my4663_viewed' | 'followed_change_viewed' | 'followed_open_loop_viewed' | 'followed_subject_return' | 'share_clicked' | 'share_link_copied' | 'share_native_completed', loopId?: string) {
  void fetch(toApiUrl(getApiBaseUrl(), '/v1/4663/campaign/events'), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ event, surface: 'home', ...(event.startsWith('share_') ? { share_object_id: loopId } : { window_id: loopId }) }) }).catch(() => undefined);
}
