  ...
  return <div className="shell narrative-shell rh-chain-shell meme-pulse-shell">
    ...
    <main id="meme-pulse-content" className="narrative-page rh-chain-page meme-pulse-page">
      ...
      {pulse && envelope && <>
        ...
        <section className="meme-pulse-section meme-pulse-risk-strip" aria-labelledby="launchpad-stress"><div className="rh-chain-section-head"><div><p className="section-kicker">Launchpad Stress</p><h2 id="launchpad-stress">Launch-surface fragmentation watch</h2><p>Context is tracked conservatively while primary evidence catches up.</p></div></div><div className="meme-pulse-risk-grid">{pulse.launchpad_stress.map((item) => <article key={item.id}><span className={`meme-pulse-risk risk-${item.risk_state}`}>{item.risk_state.replaceAll('_', ' ')}</span><h3>{item.title}</h3><p>{item.summary}</p></article>)}</div></section>
