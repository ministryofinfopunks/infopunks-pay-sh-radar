...
describe('IPX / PLTR Preflight Lab API', () => {
  ...
  it('documents research-only shadow and capacity operations—no execution endpoint', async () => {
    const app = await createApp(); apps.push(app); const spec = (await app.inject('/openapi.json')).json(); const prefix = '/v1/4663/reflexive/preflight/ipx-pltr'; expect(spec.paths[`${prefix}/simulate`]?.post).toBeTruthy(); expect(spec.paths[`${prefix}/simulations/{id}`]?.get).toBeTruthy(); expect(spec.paths[`${prefix}/shadow/candidates`]?.get).toBeTruthy(); expect(spec.paths[`${prefix}/shadow/replay`]?.post).toBeTruthy(); expect(spec.paths[`${prefix}/shadow/series/{configuration_hash}`]?.get).toBeTruthy(); expect(spec.paths[`${prefix}/capacity-sweeps`]?.post).toBeTruthy(); const paths = Object.keys(spec.paths).filter((path) => path.startsWith(prefix)); expect(paths.join(' ')).not.toMatch(/deploy|approve|swap|trade|reserve|transaction/i);
  });
});
