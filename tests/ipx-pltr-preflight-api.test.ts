...
describe('IPX / PLTR Preflight Lab API', () => {
  ...
  it('documents only simulate/read operations—no deployment, approval, swap, reserve or transaction endpoint', async () => {
    const app = await createApp(); apps.push(app); const spec = (await app.inject('/openapi.json')).json(); const prefix = '/v1/4663/reflexive/preflight/ipx-pltr'; expect(spec.paths[`${prefix}/simulate`]?.post).toBeTruthy(); expect(spec.paths[`${prefix}/simulations/{id}`]?.get).toBeTruthy(); const paths = Object.keys(spec.paths).filter((path) => path.startsWith(prefix)); expect(paths).toEqual([`${prefix}/simulate`, `${prefix}/simulations/{id}`]); expect(paths.join(' ')).not.toMatch(/deploy|approve|swap|trade|reserve|transaction/i);
  });
});
