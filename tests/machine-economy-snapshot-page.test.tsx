...
describe('machine economy snapshot page', () => {
  ...
  it('renders the snapshot route with computed headline values and current caveats', async () => {
    ...
    expect(text).toContain('Machine Market Radar Snapshot');
    expect(text).toContain('Machine services are becoming spendable. Spendable does not mean safe.');
    expect(text).toContain('Radar inserts policy, proof, and memory before autonomous action.');
    expect(text).toContain('listed ≠ callable');
    expect(text).toContain('callable ≠ executed');
    expect(text).toContain('credentials ≠ payment proof');
    expect(text).toContain('route surface ≠ receipt');
    expect(text).toContain('machine markets need judgment before spend');
    expect(stats).toContain('Services mapped13');
    expect(stats).toContain('Categories8');
    expect(text).toContain('10 allow / 2 review / 1 deny');
    expect(stats).toContain('First-safe queueactive');
    expect(stats).toContain('Market-wide execution claim0');
    expect(stats).toContain('Service-specific execution receipts0');
    expect(stats).toContain('Payment success claims0');
    expect(stats).toContain('Benchmark claims0');
    expect(stats).toContain('Winner claims0');
  ...
  it('renders all 13 services, category summary, public brief, copy button, and methodology drawer', async () => {
    ...
    expect(container.querySelector('[aria-label="Machine economy snapshot caveats"] a[href="/machine-rail-coverage"]')?.textContent).toContain('View rail coverage');
    expect(container.querySelector('[aria-label="Machine economy snapshot caveats"] a[href="/machine-route-risk-matrix"]')?.textContent).toContain('View route risk matrix');
    expect(container.querySelector('[aria-label="Machine economy snapshot caveats"] a[href="/machine-first-safe-routes"]')?.textContent).toContain('View first safe route queue');
