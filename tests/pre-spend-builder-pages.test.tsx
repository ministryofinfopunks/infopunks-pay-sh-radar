...
describe('pre-spend builder pages', () => {
  ...
  it('renders Wallet Safety Developer Quickstart with examples and integration guidance', async () => {
    ...
    expect(text).toContain('risk_score_id');
    expect(text).toContain('audit_trail_id');
    expect(text).toContain('agent_action_taken');
    expect(text).toContain('timestamp');
    expect(text).toContain('type AgentActionTaken =');
    expect(text).toContain('function buildWalletSafetyIntegrationReceipt(safety, agentActionTaken: AgentActionTaken)');
    expect(text).toContain('const integrationReceipt = buildWalletSafetyIntegrationReceipt(safety, action);');
    expect(text).toContain('await db.walletSafetyReceipts.insert(integrationReceipt);');
    expect(text).toContain('safe_to_spend');
    expect(text).toContain('spent');
    expect(text).toContain('test_spend_required');
    expect(text).toContain('test_spend');
    expect(text).toContain('manual_review_required');
    expect(text).toContain('queued_review');
    expect(text).toContain('block_spend');
    expect(text).toContain('blocked');
    expect(text).toContain('insufficient_evidence');
    expect(text).toContain('requested_evidence');
    expect(text).toContain('Store agent_action_taken as paused or requested_evidence.');
    expect(text).toContain('safe_to_spend');
    expect(text).toContain('test_spend_required');
    expect(text).toContain('manual_review_required');
    expect(text).toContain('block_spend');
