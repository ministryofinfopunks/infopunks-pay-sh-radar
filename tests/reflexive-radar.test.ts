...
import { CANONICAL_UNISWAP_V4_POOL_MANAGER_4663, buildPltrPreflightState, classifyPltrRelationship, PLTR_CONCENTRATION_SCENARIO_BANDS, recoverV4PoolKeyFromInitialize, UNISWAP_V4_INITIALIZE_ABI, pltrBasis, proposedFirstPartyPltrFootprintSchema, v4PriceFromSqrtPriceX96, verifyPltrV4Market } from '../src/services/rhChainPltrPreflightService';
...
describe('Reflexive Radar maths and identity guards', () => {
  ...
  it('constructs a scoped PLTR_PREFLIGHT_STATE and never invents first-party values', () => {
    ...
    const state = buildPltrPreflightState({ asset: pltr, supply: { total_supply_raw: '1000', total_supply_units: '1000', share_equivalent_supply: '1000', observed_block: 100, observed_at: '2026-09-01T00:00:00.000Z', source: 'test', freshness: 'fresh' }, pairs: [pair], inventory: [inventory({ mission_pair_id: 'pair-pltr', stock_asset_id: 'asset-pltr', stock_symbol: 'PLTR', observed_block: 100, stock_principal_raw: '200', stock_principal_units: '200', stock_decimals: 0 })], events: [], markets: [{ relationship: 'DIRECT_PRICE_DISCOVERY', freshness: 'fresh', pool_id: 'direct', pool_address: null, venue: 'V4', dex_version: 'V4', base_contract: CONTRACT, quote_contract: '0x4444444444444444444444444444444444444444', base_symbol: 'PLTR', quote_symbol: 'USDG', quote_direction_verified: false, verification_state: 'DISCOVERED_UNVERIFIED', liquidity_usd: 10, volume_24h_usd: 20, transaction_count: 1, observed_at: '2026-09-01T00:00:00.000Z', source: 'test', pool_state: null, depth_primitive: null }], basis: pltrBasis({ dex_price_usd: 100, underlying_price_usd: 100, multiplier: '1', dex_observed_at: '2026-09-01T00:00:00.000Z', reference_observed_at: '2026-09-01T00:00:00.000Z', session: 'OPEN' }), now: new Date('2026-09-01T01:00:00.000Z') })!;
    expect(state).toMatchObject({ state_type: 'PLTR_PREFLIGHT_STATE', tracked_mission_inventory: { absorption_pct: '20' }, inventory_coverage: { unclassified_raw: '800' }, readiness: { status: 'PARTIAL' } }); expect(state.first_party_footprint.values_populated).toBe(false); expect(PLTR_CONCENTRATION_SCENARIO_BANDS).toEqual([1, 3, 5, 10, 15, 25]); expect(proposedFirstPartyPltrFootprintSchema().denominator).toBe('CANONICAL_PLTR_TOTAL_SUPPLY_SAME_BLOCK');
  });
  it('derives a V4 price with decimal ordering and refuses a provider-only PLTR candidate', async () => {
    expect(v4PriceFromSqrtPriceX96({ sqrt_price_x96: 2n ** 96n, currency0_decimals: 18, currency1_decimals: 18, base_is_currency0: true })).toBe('1');
    expect(v4PriceFromSqrtPriceX96({ sqrt_price_x96: 2n ** 96n, currency0_decimals: 6, currency1_decimals: 18, base_is_currency0: true })).toBe('0.000000000001');
    const rejected = await verifyPltrV4Market({ chain_id: 4663, canonical_pltr_contract: CONTRACT, base_contract: CONTRACT, quote_contract: '0x4444444444444444444444444444444444444444', relationship: 'DIRECT_PRICE_DISCOVERY', pool_key: null, reported_pool_id: '0x' + '00'.repeat(32), state: null }); expect(rejected.status).toBe('DISCOVERED_UNVERIFIED'); expect(rejected.failures).toContain('POOL_KEY_UNRESOLVED');
  });
  it('requires synchronized USD reference data for PLTR basis', () => {
    expect(pltrBasis({ dex_price_usd: 100, underlying_price_usd: 100, multiplier: '1', dex_observed_at: '2026-09-01T00:00:00.000Z', reference_observed_at: '2026-09-01T00:01:00.000Z', session: 'OPEN' })).toMatchObject({ status: 'AVAILABLE', basis_bps: 0 });
    expect(pltrBasis({ dex_price_usd: 100, underlying_price_usd: 100, multiplier: '1', dex_observed_at: '2026-09-01T00:00:00.000Z', reference_observed_at: '2026-09-01T00:05:00.000Z', session: 'OPEN' }).status).toBe('UNAVAILABLE');
  });
  it('pins the generic Initialize ABI and reconciles the documented PLTR/USDG PoolKey', async () => {
    expect(CANONICAL_UNISWAP_V4_POOL_MANAGER_4663).toBe('0x8366a39cc670b4001a1121b8f6a443a643e40951'); expect(UNISWAP_V4_INITIALIZE_ABI[0].inputs.map((item) => item.name)).toEqual(['id', 'currency0', 'currency1', 'fee', 'tickSpacing', 'hooks', 'sqrtPriceX96', 'tick']);
    const result = await verifyPltrV4Market({ chain_id: 4663, canonical_pltr_contract: '0x894e1ec2d74ffe5aef8dc8a9e84686accb964f2a', base_contract: '0x5fc5360d0400a0fd4f2af552add042d716f1d168', quote_contract: '0x894e1ec2d74ffe5aef8dc8a9e84686accb964f2a', relationship: 'DIRECT_PRICE_DISCOVERY', pool_key: { currency0: '0x5fc5360d0400a0fd4f2af552add042d716f1d168', currency1: '0x894e1ec2d74ffe5aef8dc8a9e84686accb964f2a', fee: 10_000, tick_spacing: 200, hooks: '0x0000000000000000000000000000000000000000' }, reported_pool_id: '0xee430ee1003e1985e1828a01b9a20dad67ad4302994fe2abb4a173de4ac54623', state: { sqrt_price_x96: String(2n ** 96n), tick: 0, active_liquidity: '1', observed_block: 1, observed_at: '2026-09-02T00:00:00.000Z', state_view: CONTRACT } }); expect(result.status).toBe('VERIFIED'); expect(result.depth_primitive?.fee).toBe(10_000);
  });
  it('fails closed on an untrusted PoolManager and a mismatched recovered PoolKey', async () => {
    await expect(recoverV4PoolKeyFromInitialize({ rpc_url: 'https://rpc.invalid', pool_id: '0x' + '00'.repeat(32), pool_manager: CONTRACT })).resolves.toBeNull();
    const result = await verifyPltrV4Market({ chain_id: 4663, canonical_pltr_contract: CONTRACT, base_contract: CONTRACT, quote_contract: '0x4444444444444444444444444444444444444444', relationship: 'DIRECT_PRICE_DISCOVERY', pool_key: { currency0: CONTRACT, currency1: '0x4444444444444444444444444444444444444444', fee: 500, tick_spacing: 10, hooks: '0x0000000000000000000000000000000000000000' }, reported_pool_id: '0x' + '00'.repeat(32), state: { sqrt_price_x96: String(2n ** 96n), tick: 0, active_liquidity: '1', observed_block: 1, observed_at: '2026-09-02T00:00:00.000Z', state_view: CONTRACT } });
    expect(result).toMatchObject({ status: 'DISCOVERED_UNVERIFIED', failures: ['POOL_ID_MISMATCH'] });
  });
  it('only reaches READY from a complete immutable verified market state', () => {
    const pltr = { ...asset(), asset_id: 'asset-pltr', ticker: 'PLTR', canonical_contract: CONTRACT, current_multiplier: '1' }; const pair = { pair_id: 'pair-pltr', stock_asset_id: 'asset-pltr', canonicality: 'verified', verification: { verification_status: 'VERIFIED' }, mission_symbol: 'MISSION', venue: 'PAIR', pool_id: 'pool' } as any; const depth = { pool_key: { currency0: CONTRACT, currency1: '0x4444444444444444444444444444444444444444', fee: 500, tick_spacing: 10, hooks: '0x0000000000000000000000000000000000000000' }, pool_id: '0x' + '11'.repeat(32), sqrt_price_x96: String(2n ** 96n), tick: 0, active_liquidity: '1', fee: 500, tick_spacing: 10, hooks: '0x0000000000000000000000000000000000000000', currency0: CONTRACT, currency1: '0x4444444444444444444444444444444444444444', observed_block: 100, observed_at: '2026-09-01T00:00:00.000Z', state_view: CONTRACT, initialized: true, methodology: 'V4_STATEVIEW_DEPTH_PRIMITIVE_V1' } as const;
    const input = { asset: pltr, supply: { total_supply_raw: '1000', total_supply_units: '1000', share_equivalent_supply: '1000', observed_block: 100, observed_at: '2026-09-01T00:00:00.000Z', source: 'test', freshness: 'fresh' as const }, pairs: [pair], inventory: [inventory({ mission_pair_id: 'pair-pltr', stock_asset_id: 'asset-pltr', stock_symbol: 'PLTR', observed_block: 100, stock_principal_raw: '20', stock_decimals: 0 })], events: [], markets: [{ relationship: 'DIRECT_PRICE_DISCOVERY' as const, freshness: 'fresh' as const, pool_id: depth.pool_id, pool_address: null, venue: 'Uniswap', dex_version: 'V4', base_contract: CONTRACT, quote_contract: depth.currency1, base_symbol: 'PLTR', quote_symbol: 'USDG', quote_direction_verified: false, verification_state: 'VERIFIED' as const, liquidity_usd: 10, volume_24h_usd: 10, transaction_count: 1, observed_at: depth.observed_at, source: 'test', pool_state: null, depth_primitive: depth }], basis: pltrBasis({ dex_price_usd: 100, underlying_price_usd: 100, multiplier: '1', dex_observed_at: depth.observed_at, reference_observed_at: depth.observed_at, session: 'OPEN' }), now: new Date('2026-09-01T01:00:00.000Z') };
    const ready = buildPltrPreflightState(input)!; expect(ready.readiness.status).toBe('READY_FOR_SIMULATION'); expect(ready.immutable).toBe(true); expect(buildPltrPreflightState({ ...input, supply: null })!.readiness.status).toBe('NOT_READY');
  });
  it('preserves unknown mission inventory as partial coverage rather than a zero balance', () => {
    const pltr = { ...asset(), asset_id: 'asset-pltr', ticker: 'PLTR', canonical_contract: CONTRACT, current_multiplier: '1' };
    const state = buildPltrPreflightState({ asset: pltr, supply: { total_supply_raw: '1000000000000000000000', total_supply_units: '1000', share_equivalent_supply: '1000', observed_block: 100, observed_at: '2026-09-01T00:00:00.000Z', source: 'test', freshness: 'fresh' }, pairs: [], inventory: [], events: [] })!;
