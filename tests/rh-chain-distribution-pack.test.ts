...
describe('RH Chain Distribution Pack', () => {
  ...
  it('exports caveated Receipt Relay packets without coordination or market-promise language', () => {
    ...
    expect(relay.packets.map((packet) => packet.title)).toEqual(expect.arrayContaining(['Daily Receipt latest', 'Meme Pulse', 'Launchpad Observatory', 'Clone Radar', 'Token Dossier source-required state', 'Scout Agent answer excerpt']));
