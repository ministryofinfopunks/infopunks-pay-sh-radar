    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      ...
      if (path === '/v1/radar/mapping-targets') return json({
        ...
        targets: [
          ...
          { category: 'finance/data', benchmark_intent: 'token search', current_state: 'second_verified_mapping_found', needed_next_step: 'Run paid execution for StableCrypto token-search route.', suggested_provider_candidates: ['CoinGecko Onchain DEX API', 'StableCrypto'], why_it_matters: 'Search intent supports benchmark input shaping.', readiness_blocker: 'PaySponge has proven execution, but StableCrypto is verified/unproven. Benchmark readiness requires two comparable proven routes.' },
