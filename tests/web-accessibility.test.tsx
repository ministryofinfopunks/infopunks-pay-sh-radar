...
describe('web accessibility landmarks', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      ...
      if (path === '/v1/radar/mapping-targets') return json({
        ...
        count: 5,
        targets: [
          { category: 'finance/data', benchmark_intent: 'token metadata', current_state: 'candidate_mapping_found', needed_next_step: 'verify endpoint/method/request shape for token metadata candidates', suggested_provider_candidates: ['CoinGecko Onchain DEX API', 'StableCrypto'], why_it_matters: 'Token metadata is needed for normalization before comparisons.', readiness_blocker: 'candidate mappings exist, but no verified/proven token metadata route evidence is recorded' },
  ...
  it('renders semantic landmarks and important accessible control names', async () => {
    ...
    expect(container.querySelector('nav[aria-label="Solana navigation"]')).not.toBeNull();
    ...
    expect(container.querySelector('section[aria-labelledby="explore-solana-radar-title"]')).not.toBeNull();
    expect(Array.from(container.querySelectorAll('a[href="#global-pulse"]')).some((link) => link.textContent?.includes('Read the live signal'))).toBe(true);
    expect(container.querySelector('a[href="/rh-chain-signal-desk"] strong')?.textContent).toBe('Robinhood Chain');
